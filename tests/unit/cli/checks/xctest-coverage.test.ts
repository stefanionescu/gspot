...
test('the coverage check compares each named target with its floor', () => {
    ...
    expect(underFloor(report, [{ target: 'App', percent: 60 }])).toStrictEqual([]);
    expect(underFloor(report, [{ target: 'App', percent: 80 }])).toStrictEqual([
        'App covers 61 of 100 lines, under the floor of 80.',
    ]);
    expect(underFloor(report, [{ target: 'Widget', percent: 10 }])).toStrictEqual([
