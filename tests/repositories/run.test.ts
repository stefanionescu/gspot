...
const gspot = join(root, 'packages', 'cli', 'src', 'main.ts');

type Outcome = { code: number; stdout: string; stderr: string };

function run(cwd: string, args: string[], env: Record<string, string> = {}): Outcome {
    const result = Bun.spawnSync(['bun', gspot, ...args], {
        cwd,
...
describe('the bash planted repository', () => {
    test('init --yes writes the policy and check passes over a clean script', async () => {
        ...
        const drift = run(fixture.path, ['sync', '--check']);
    ...
    test('a finding fails the check with the file, the rule and a help line', async () => {
    ...
    test('a version pin from another gspot refuses check with both remedies', async () => {
    ...
    test('the pre-commit hook runs the staged checks', async () => {
        await using fixture = await createFixture({ 'scripts/a.sh': script });
        git(fixture.path, ['init', '-q']);
        git(fixture.path, ['add', '-A']);
        git(fixture.path, ['commit', '-qm', 'init']);
        ...
        git(fixture.path, ['add', '-A']);
        const commit = Bun.spawnSync(['git', '-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-qm', 'bad'], {
            cwd: fixture.path,
            env: { ...process.env, GSPOT_BIN: `bun ${gspot}`, NO_COLOR: '1' },
            stdout: 'pipe',
            stderr: 'pipe',
        });
        expect(commit.exitCode).not.toBe(0);
        expect(`${commit.stdout.toString()}${commit.stderr.toString()}`).toContain('SC2086');
    });
});
