...
test(
    ...
    async () => {
        ...
        expect(
            reportSchema
                .parse(JSON.parse(failed.stdout))
        ...
        for (const folder of ['', 'app']) {
            const native = Bun.spawnSync([join(sandbox.path, '.gspot/node_modules/.bin/stylelint'), 'site.css'], {
                cwd: join(sandbox.path, folder),
                stdout: 'pipe',
                stderr: 'pipe',
            });
            expect(native.exitCode, native.stdout.toString() + native.stderr.toString()).toBe(0);
        }
    },
...
test.each([
    ...
    async ({ scope, inherited }) => {
        ...
        const failed = await run(sandbox.path, ['check', '--only', 'css/stylelint', '--no-cache'], environment);
        expect(failed.code, failed.stdout + failed.stderr).toBe(1);
        expect(failed.stdout).toContain('color-named');
        expect(failed.stdout).toContain('selector-max-id');
        await Bun.write(join(sandbox.path, prefix, 'future.css'), 'a {\n    color: #abc;\n}\n');
        const corrected = await run(sandbox.path, ['check', '--only', 'css/stylelint', '--no-cache'], environment);
        expect(corrected.code, corrected.stdout + corrected.stderr).toBe(0);
