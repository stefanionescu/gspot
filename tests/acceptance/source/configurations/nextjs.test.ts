...
async function installedNextProject() {
    // Webpack requires the linked dependencies and the sandbox to share a drive.
    const sandbox = await testdir({}, { dirname: join(join(MODULES, '../..'), 'gspot-test-' + randomUUID()) });
    try {
        await createFileTree(sandbox.path, {
...
describe('the nextjs and i18n configurations', () => {
    test(
        ...
        async () => {
            ...
            await Bun.write(join(sandbox.path, 'app/layout.tsx'), LAYOUT);
