...
beforeAll(async () => {
    ...
    toolNpmrc = join(registry.work, 'tools.npmrc');
    writeFileSync(
        toolNpmrc,
...
async function initialize(fixture: Awaited<ReturnType<typeof installedConsumer>>) {
...
test(
    ...
    async () => {
        await using fixture = await installedConsumer();
