...
const entry = fileURLToPath(new URL('../ci-affected.ts', import.meta.url));
...
test('repository CI checks the committed change, preserves reports on invalid bases, and accepts a correction', async () => {
    ...
    const execute = (comparison: string) =>
        run([process.execPath, entry], { cwd: sandbox.path, env: { GSPOT_CI_BASE: comparison }, timeoutMs: 30_000 });
