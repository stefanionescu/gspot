...
function launcher(
    is64: boolean,
    kind = 1,
    interpreter = 'C:\\working project\\.venv\\Scripts\\python.exe',
): Buffer<ArrayBuffer> {
    ...
    bytes.writeUInt32LE(0x80000000 + 24, 512 + 20);
    bytes.writeUInt16LE(3, 512 + 24 + 12);
    let nameOffset = 184;
    let dataOffset = 512;
    const resources: [string, Buffer][] = [
        ['UV_PYTHON_PATH', Buffer.from(interpreter)],
        ['UV_SCRIPT_DATA', Buffer.from('PK\x03\x04embedded script\x00\xff', 'latin1')],
    ...
    for (const [index, [name, content]] of resources.entries()) {
        ...
        bytes.writeUInt32LE(0x80000000 + nameOffset, 512 + 40 + index * 8);
        bytes.writeUInt32LE(0x80000000 + 64 + index * 24, 512 + 44 + index * 8);
...
const INTERPRETERS = new Map([
    ['C:\\working project\\.venv\\Scripts\\python.exe', 'C:\\selected revision\\.venv\\Scripts\\python.exe'],
]);
...
test.each([false, true])('uv launcher relocation preserves resource payloads in PE64=%s', (is64) => {
    ...
    expect(relocated.subarray(512 + 144, original.length)).toEqual(original.subarray(512 + 144));
    expect(relocated.readUInt32LE(added + 36)).toBe(0x40000040);
});

test('uv interpreter trampolines retain their declared host interpreter', () => {
    expect(
        relocateWindowsLauncher(
            launcher(true, 2, 'C:\\Python\\python.exe'),
            INTERPRETERS,
            new Set(['C:\\Python\\python.exe']),
        ),
    ).toBeUndefined();
});
...
test('revision snapshots relocate uv PE resources and retain the working launcher', async () => {
    ...
    await withRevisionSnapshot(root, { kind: 'index' }, async (snapshot) => {
        ...
        expect(bytes.subarray(512 + 144, original.length)).toEqual(original.subarray(512 + 144));
    ...
    expect(readFileSync(join(root, '.venv/Scripts/check.exe'))).toEqual(original);
    expect(readFileSync(join(root, '.venv/Scripts/python.exe'))).toEqual(interpreter);
    expect(readFileSync(hostPath, 'utf8')).toBe('Declared native host interpreter');
});

test.each(['python.exe', '.\\PYTHON.EXE', '..\\Scripts\\python.exe'])(
...
test.each(['..\\..\\outside\\python.exe', 'C:python.exe', '\\python.exe'])(
...
test('relative uv host interpreters relocate only to declared host executables', () => {
    const host = 'C:\\Python\\python.exe';
    const original = launcher(true, 2, '..\\..\\..\\Python\\python.exe');
    ...
    expect(() => relocateWindowsLauncher(original, INTERPRETERS, new Set(['C:\\Elsewhere\\python.exe']))).toThrow(
        'Cannot relocate',
    );
});
