...
test('shared output readers reject external links without changing their targets', async () => {
    ...
    expect(() => isMergeStubHeld(project, stub, stub.path, '.gspot/tsconfig.json')).toThrow('private regular file');
    expect(() =>
...
test.each(['{"extends":"./.gspot/tsconfig.json", invalid}', 'null', '[]'])(
    ...
    async (content) => {
        ...
        expect(() => isMergeStubHeld(sandbox.path, stub, stub.path, '.gspot/tsconfig.json')).toThrow(
            'valid JSON object',
        );
