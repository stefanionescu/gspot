    ...
    for (const [index, line] of text.split(/\r?\n/u).entries()) {
        ...
        while (remaining !== '' && !remaining.startsWith('#')) {
            ...
            if (directive === null || key === undefined)
                throw new Error(`Invalid ShellCheck directive on line ${String(index + 1)}.`);
            ...
            if (quoted && end < 0) throw new Error(`Unterminated ShellCheck quote on line ${String(index + 1)}.`);
            if (quoted && end === 1) throw new Error(`Empty ShellCheck quote on line ${String(index + 1)}.`);
            ...
            if (key === 'enable' || key === 'disable') {
                ...
                rules[key].push(
                    ...entries.map((entry) =>
                        key === 'disable'
                            ? entry.replaceAll(/(?:SC)?(\d+)/gu, (_, code: string) => `SC${String(Number(code))}`)
                            : entry,
                    ),
                );
            }
