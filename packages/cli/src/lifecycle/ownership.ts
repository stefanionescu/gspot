...
const entrySchema = z.strictObject({
    path: pathSchema,
    kind: z.enum(['config', 'block', 'merge', 'policy', 'pin', 'hook', 'lock', 'dependency', 'runtime']),
...
export function generatedSnapshot(file: GeneratedFile, current: FileSnapshot | undefined): FileSnapshot {
    ...
    const mode = file.executable === true ? 0o755 : file.readOnly ? 0o444 : 0o644;
    const checkout =
        mode === 0o444 && current?.mode === 0o644 && current.isLink !== true && current.bytes.equals(bytes);
...
function configurationDocument(source: string, format: 'json' | 'yaml', created = false) {
    if (format === 'yaml') {
    ...
    try {
        ...
        const applyProposals: LifecycleOwner['applyProposals'] = (proposals) => {
            ...
            for (const proposal of proposals) {
                ...
                const existing = find(path);
                if (next !== undefined) confined.validate(path, next);
            ...
            const prepared = proposals.flatMap((proposal) => {
                ...
                return [{ path, current, next, entry: recordedEntry }];
            });
            if (prepared.length === 0) return statuses;
            state.pending = prepared.map(({ path, current, next, entry }) => ({
                path,
            ...
            save();
            for (const { path, current, next } of prepared) {
                if (next === undefined) {
