...
import type { EslintAdoption, EslintRegistration } from '#cli/policy/types.ts';
import type { LegacyEslintApi, LegacyEslintCriteria } from '#cli/lifecycle/types.ts';
...
async function legacyEntries(
    ...
    for (const directory of directories) {
        let source: import('#cli/lifecycle/types.ts').LegacyEslintEntry[] = [];
...
export async function evaluateRuleCoverage(
    ...
    if (openConfinedRoot(request.root).read('.gspot/eslint.config.mjs') === undefined)
    ...
    const eslint = new Constructor({
        cwd: request.root,
        overrideConfigFile: join(request.root, '.gspot/eslint.config.mjs'),
    });
