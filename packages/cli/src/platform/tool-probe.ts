...
function candidates(root: string, roots: string[], name: string, privateKind?: 'npm' | 'python'): string[] {
    ...
    const directories =
        privateKind === 'npm'
            ? [join(root, '.gspot/node_modules/.bin')]
            : privateKind === 'python'
              ? [join(root, '.gspot/.venv', isWindows ? 'Scripts' : 'bin')]
...
function probeLibrary(root: string, tool: ToolPin): ToolProbe {
    ...
    const path = `.gspot/node_modules/${name}/package.json`;
...
export function locateTool(root: string, name: string): string | undefined {
    ...
    const tool = toolPin(presetManifests().values(), name);
