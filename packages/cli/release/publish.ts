...
// Usage: bun packages/cli/release/publish.ts --tag v0.1.0 [--registry <url>] [--dry-run]
...
try {
    const script = new Command('bun packages/cli/release/publish.ts')
        ...
        .addHelpText(
            'after',
            `\nExample: bun packages/cli/release/publish.ts --tag v${packageManifest.version} --dry-run`,
        )
        .exitOverride()
        .parse();
