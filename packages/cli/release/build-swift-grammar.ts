        ...
        if (check) {
            for (const file of files)
                if (!readFileSync(join(out, file)).equals(readFileSync(join(work, file))))
                    throw new Error(`Regenerate ${file} with bun packages/cli/release/build-swift-grammar.ts.`);
        } else {
...
try {
    const program = new Command('bun packages/cli/release/build-swift-grammar.ts')
        .description('Build the pinned Swift grammar with tree-sitter and an isolated Emscripten container')
        .option('--out <directory>', 'Output directory', join(here, '../vendor'))
