...
const upstream = z
    ...
    .parse(JSON.parse(readFileSync(new URL('../vendor/notices.json', import.meta.url), 'utf8')));
...
export function binaryNotices(dependencies: string, grammarDirectory: string): string {
    ...
    if (digest !== provenance.sha256)
        throw new Error(
            'The Swift grammar does not match its provenance. Run bun packages/cli/release/build-swift-grammar.ts.',
        );
