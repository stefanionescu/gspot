...
const upstream = z
    .object({
        licenses: z.record(z.string(), z.string()),
        packages: z.record(z.string(), recordSchema),
        swift: z.object({ version: z.string(), url: z.url(), sha256: z.string().regex(/^[a-f0-9]{64}$/u) }),
    })
    .refine(
...
export function binaryNotices(dependencies: string): string {
    ...
    return `# Third-party notices\n\nGenerated by bun packages/cli/release/build.ts from bundled dependency licenses and pinned upstream records.\n\n${dependencies}\n## tree-sitter-swift ${upstream.swift.version}\n\nSource: ${upstream.swift.url}\n\nSHA-256: ${upstream.swift.sha256}\n${upstreamNotice(`tree-sitter-swift@${upstream.swift.version}`)}\n## Bun ${Bun.version}\n\nSource: ${runtime.source}\n\n${upstream.licenses[runtime.license]!}`;
}
