...
test('mise executes generated tasks with their arguments, and install rejects an old runner before corrected setup succeeds', async () => {
    ...
    const rejected = await run([process.execPath, CLI, 'set', 'runner.tool', 'unsupported', '--json'], { cwd: repository.path });
    ...
    const selected = await run([process.execPath, CLI, 'set', 'runner.tool', 'mise', '--json'], { cwd: repository.path });
