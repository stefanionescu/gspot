    ...
    describe(`framework output preservation in ${scope || 'root'}`, () => {
        for (const check of ['nextjs/typecheck', 'nextjs/build']) {
            test(`${check} reports a defect, accepts its correction, and preserves source output`, async () => {
                ...
                try {
                    const execute = check === 'nextjs/typecheck' ? frameworkTypes : frameworkBuild;
...
test('failed type generation cleans the isolated copy without restoring over source edits', async () => {
    ...
    const run = spyOn(processes, 'run').mockImplementation(async (_command, options) => {
        ...
        return { code: 1, missing: false, duration: 1, stdout: '', stderr: 'Error: Generator failed' };
    });
    try {
        await expect(frameworkTypes(input)).rejects.toThrow('Generator failed');
