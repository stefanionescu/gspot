...
test('inline ignores apply to Swift engine findings on fresh and cached runs', async () => {
    ...
    const failed = await executeRun(await openSession(fixture.path), options);
    ...
    const path = join(fixture.path, 'Sources/Welcome.swift');
    ...
    const unexplained = await executeRun(await openSession(fixture.path), options);
...
test('inline engine comments do not suppress external-tool findings', async () => {
    ...
    expect(applyInlineIgnores(fixture.path, [finding])).toEqual([finding]);
});
