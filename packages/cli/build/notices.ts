...
export async function binaryNotices(dependencies: string): Promise<string> {
    ...
    return `# Third-party notices\n\nGenerated by bun packages/cli/build/command.ts from bundled dependency licenses and pinned upstream records.\n\n${dependencies}\n## tree-sitter-swift ${SWIFT_GRAMMAR.version}\n\nSource: ${SWIFT_GRAMMAR.url}\n\nSHA-256: ${SWIFT_GRAMMAR.sha256}\n${await upstreamNotice(`tree-sitter-swift@${SWIFT_GRAMMAR.version}`)}\n## Bun ${Bun.version}\n\nSource: ${runtime.source}\n\n${await upstreamNotice(`bun@${Bun.version}`)}`;
}
