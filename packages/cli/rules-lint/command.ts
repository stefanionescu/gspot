...
const vale = binary === undefined ? {} : { vale: { binary, config: join(root, 'prose', 'vale.ini') } };
const report = lintRules(files, vale, root);
for (const finding of report.findings) console.log(`rules/${finding.file}:${String(finding.line)}: ${finding.message}`);
if (!report.isValeRun) console.error('vale is not installed; the prose rules of the corpus lint did not run');
