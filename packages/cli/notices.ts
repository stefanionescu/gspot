...
const upstreamRecords = z
    .record(z.string(), recordSchema)
    .parse(JSON.parse(readFileSync(join(licenses, 'records.json'), 'utf8')));
const metadataSchema = z.object({ inputs: z.record(z.string(), z.unknown()) });
const MINIMUM_FENCE_LENGTH = 3;
const LICENSE = /^(?:LICENSE|LICENCE|COPYING|NOTICE)(?:$|[.-])/iu;
...
function upstreamNotice(identity: string): string {
    const record = upstreamRecords[identity];
    if (record === undefined) throw new Error(`No license notice is recorded for bundled ${identity}.`);
    let text = readFileSync(join(licenses, record.file), 'utf8');
...
export function binaryNotices(dependencies: string, grammarDirectory: string): string {
    ...
    if (digest !== provenance.sha256)
        throw new Error(
            'The Swift grammar does not match its provenance. Run bun packages/cli/grammars/swift.build.ts.',
        );
    const runtime = upstreamRecords[`bun@${Bun.version}`];
    if (runtime === undefined) throw new Error(`No runtime notice is recorded for Bun ${Bun.version}.`);
    return `# Third-party notices\n\nGenerated by bun packages/cli/build.ts from bundled dependency licenses and pinned upstream records.\n\n${dependencies}\n## tree-sitter-swift ${provenance.version}\n\nSource: ${provenance.source}\n\nRevision: ${provenance.revision}\n\nSHA-256: ${provenance.sha256}\n\n${licenseBlock('Swift grammar license', readFileSync(join(grammarDirectory, 'swift.LICENSE'), 'utf8'))}\n## Bun ${Bun.version}\n\nSource: ${runtime.source}\n\n${readFileSync(join(licenses, runtime.file), 'utf8')}`;
}
