...
Prefer explicit prefetching for routes where nested queries otherwise wait on one another. If
...
Use a stable selector reference when a costly transformation otherwise reruns on every render.
...
- When removing a cache entry, expect pagination to restart from its initial page. Do not keep a
  global cursor pointing beyond pages that the query does not hold.
