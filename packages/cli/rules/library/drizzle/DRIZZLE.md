...
Use a database-generated column when PostgreSQL computes the value from the row. Verify
...
- Check the affected rows or a suitable `returning` projection. Handle a row that disappeared or
  changed after the caller read it. Do not report success merely because the query completed.
...
3. Plan data movement. For populated tables, backfill and validate values before enforcing a new
   requirement. Use bounded batches when a single update holds locks for too long.
...
- When creating tables through Drizzle, inspect the generated migration for actual RLS enablement
  and policies. A table declaration does not enable RLS by itself.
