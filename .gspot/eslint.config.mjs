...
const policyRules = [].map(({ scope, includes, excludes, rules }) => {
    const included = includes.map((source) => new RegExp(source));
    const excluded = excludes.map((source) => new RegExp(source));
    const matches = (file) => {
...
for (const entry of adopted) {
    for (const [name, reference] of Object.entries(entry.plugins ?? {})) {
        const imported = await import(pathToFileURL(requireProject.resolve(reference.module)).href);
        entry.plugins[name] = reference.export.split('.').reduce((value, key) => value[key], imported);
    }
    if (entry.languageOptions?.parser) {
        const reference = entry.languageOptions.parser;
        const imported = await import(pathToFileURL(requireProject.resolve(reference.module)).href);
        entry.languageOptions.parser = reference.export.split('.').reduce((value, key) => value[key], imported);
    }
}
