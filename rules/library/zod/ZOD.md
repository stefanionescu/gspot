...
Use Zod to define runtime contracts for data entering or leaving the application. Keep each schema `enforced-by: typescript/eslint zod/require-strict`
...
Check the installed Zod version and the integrations that consume its schemas, including tRPC, form `enforced-by: typescript/eslint zod/require-strict`
...
- Prefer native format schemas such as `z.email()`, `z.uuid()`, and `z.iso.datetime()` when their
  ...
  validate its resolved value instead of introducing the deprecated `z.promise()` API. `enforced-by: typescript/eslint zod/require-strict`
- Infer application types from the runtime schema. Use `z.input<typeof Schema>` for values before
  ...
  defaults and resolver input types consistent when coercion or transforms change the output type. `enforced-by: typescript/eslint zod/require-strict`
- When a generated or external type owns the contract, use `z.toZod<ExternalType>()(schema)` in Zod
  ...
  supplies missing runtime validation. `enforced-by: typescript/eslint zod/require-strict`
- Use `z.custom<T>()` only with a predicate that establishes the promised type. A type argument
  ...
  ordinary data. `enforced-by: typescript/eslint zod/require-strict`
- Consider `zod/mini` when measured browser bundle size warrants its functional API. Check resolver
  compatibility and locale setup. Keep the chosen API consistent within shared schema modules. `enforced-by: typescript/eslint zod/require-strict`

### Parse external input

Validate Route Handler bodies, Server Action arguments, tRPC input, URL values, environment `enforced-by: typescript/eslint zod/require-strict`
...
- Use `.safeParse()` for expected invalid input and branch on `result.success`. Pass `result.data`
  ...
  handling, such as rejecting invalid startup configuration. `enforced-by: typescript/eslint zod/require-strict`
- Use `.safeParseAsync()` or `.parseAsync()` whenever the schema includes asynchronous refinements
  ...
  thrown by custom code, failed network requests, or malformed JSON decoding before validation. `enforced-by: typescript/eslint zod/require-strict`
- Use Zod 4.5 `z.validate(schema, input)` only when a boolean answer is sufficient. It narrows the
  ...
  or transformed data. `enforced-by: typescript/eslint zod/require-strict`
- Parse transport syntax separately from the value schema. Handle invalid JSON as a client input
  ...
  requires them before decoding the authenticated payload. `enforced-by: typescript/eslint zod/require-strict`
- Enforce request and upload size limits before buffering large inputs. Add appropriate string,
  ...
  does not establish an application's resource budget. `enforced-by: typescript/eslint zod/require-strict`
- Validate provider responses before returning them from a query function or caching them. Treat an
  ...
  that the user entered invalid data. `enforced-by: typescript/eslint zod/require-strict`
...
- Define the fields that the operation accepts. Derive a patch from an explicit selection of
  ...
  `.partial()` and let callers supply ownership, privilege, or server-managed fields. `enforced-by: typescript/eslint zod/require-strict`
- Give omission, `undefined`, and `null` separate meanings. For a patch, an omitted key normally
  ...
  `undefined`; `.nullable()` permits `null`; `.nullish()` permits both. `enforced-by: typescript/eslint zod/require-strict`
...
Define normalization in the shared contract so browser feedback and server parsing agree. Treat URL `enforced-by: typescript/eslint zod/require-strict`
...
- Extract allowed form fields deliberately. Use `getAll()` for repeated values and validate their
  ...
  framework form metadata out of a strict domain schema. `enforced-by: typescript/eslint zod/require-strict`
- Do not use `z.coerce.boolean()` to interpret the strings `"true"` and `"false"`: JavaScript treats
  ...
  explicit mapping for checkbox presence. Reject unknown spellings instead of guessing. `enforced-by: typescript/eslint zod/require-strict`
- Handle empty and absent numeric input before coercion. JavaScript converts `""` and `null` to
  ...
  numeric prefix followed by invalid text. `enforced-by: typescript/eslint zod/require-strict`
...
- Use `z.file()` for declared size and MIME constraints where supported. Validate actual file
  ...
  authorization, or ownership. Keep those checks with the responsible integration. `enforced-by: typescript/eslint zod/require-strict`

### Compose schemas and refinements

Keep schema construction outside repeated renders and request loops when it depends only on stable `enforced-by: typescript/eslint zod/require-strict`
...
- Prefer object composition over intersections when callers need `.pick()`, `.omit()`, or other
  ...
  too: assignability alone does not prevent weakening a field. `enforced-by: typescript/eslint zod/require-strict`
- Use shape spreading to compose large unrefined objects without long `.extend()` chains. Choose the
  ...
  preserve those rules deliberately when deriving a contract. `enforced-by: typescript/eslint zod/require-strict`
- Use `z.discriminatedUnion()` for variants with a stable tag. Preserve required discriminators in
  ...
  tag instead of indexing internal schema definitions. `enforced-by: typescript/eslint zod/require-strict`
- Remember that an ordinary union accepts the first successful branch. Use `z.xor()` only when
  ...
  stripped. Prefer a discriminator when it expresses the domain clearly. `enforced-by: typescript/eslint zod/require-strict`
- Use `z.record(enumSchema, valueSchema)` for a complete set of enum keys and `z.partialRecord()`
  ...
  pattern-key behavior changed in Zod 4.5. Cover accepted and rejected keys in contract fixtures. `enforced-by: typescript/eslint zod/require-strict`
...
Use `z.codec()` when a boundary needs a defined conversion in both directions, such as an ISO `enforced-by: typescript/eslint zod/require-strict`
...
- Parse unknown input with `.parse()` or `.safeParse()`. Codec `.decode()` and `z.decode()` perform
  ...
  unchecked network response. Use `.encode()` or `z.encode()` for the reverse conversion. `enforced-by: typescript/eslint zod/require-strict`
- Use safe and asynchronous codec variants according to the boundary's failure handling and the
  ...
  only in the forward direction. `enforced-by: typescript/eslint zod/require-strict`
...
Return a stable, serializable error contract from Server Actions and HTTP endpoints. Expose field `enforced-by: typescript/eslint zod/require-strict`
...
- Use `z.flattenError()` for flat forms and `z.treeifyError()` when nested objects or array indices
  ...
  form-level failures by returning only field errors. Access optional tree branches safely. `enforced-by: typescript/eslint zod/require-strict`
- Use `.issues` when building a project-specific error response. Prefer `z.treeifyError()` over
  ...
  treating its display string as a machine-readable response contract. `enforced-by: typescript/eslint zod/require-strict`
...
Use `z.toJSONSchema()` when an API description, form generator, or structured-output integration `enforced-by: typescript/eslint zod/require-strict`
...
- Keep failures for unrepresentable types visible. Dates, bigints, Maps, Sets, and custom transforms
  ...
  generation pass while silently removing constraints. Make each supported conversion explicit. `enforced-by: typescript/eslint zod/require-strict`
- Treat `z.fromJSONSchema()` as experimental. Check its installed behavior and supported keywords;
  conversion does not establish equivalence with every original validation rule. `enforced-by: typescript/eslint zod/require-strict`
- Attach stable documentation metadata with `.meta()`. Metadata can override generated JSON Schema
  ...
  values out of metadata and shared registries. `enforced-by: typescript/eslint zod/require-strict`
- Retain the schema instance returned by `.meta()` or `.describe()`. Metadata belongs to that
  ...
  schema intended for registry-based export. `enforced-by: typescript/eslint zod/require-strict`
- Verify generated required fields, unknown-field behavior, nullability, and references against
  ...
  changes and check the actual consumer's supported dialect and formats. `enforced-by: typescript/eslint zod/require-strict`

### Compile frequently used schemas

Use Zod 4.5 compilation when validation measurements justify it. Compile a reusable schema once `enforced-by: typescript/eslint zod/require-strict`
...
- Unsupported schemas can fall back without compiling. Async parsing and encoding use the normal
  parser. Use `strict: true` during verification when compilation itself is a requirement. `enforced-by: typescript/eslint zod/require-strict`
- Compilation uses `new Function()`. Verify CSP and deployment restrictions; preserve those
  ...
  generation and can fall back when blocked. `enforced-by: typescript/eslint zod/require-strict`
- Use `import "zod/compile"` only from an application-owned entry point, before modules construct
  ...
  a shared library never enables global compilation for its consumers. `enforced-by: typescript/eslint zod/require-strict`
- Measure bundle size, initialization, and representative successful and failed parses. Compiler
  ...
  output and issue behavior in the deployed runtime. `enforced-by: typescript/eslint zod/require-strict`
...
- Datetimes with omitted seconds, explicit offsets, or local values. `enforced-by: typescript/eslint zod/require-strict`
- Emoji, combining marks, and other non-ASCII strings near length limits. `enforced-by: typescript/eslint zod/require-strict`
- Omitted fields, explicit `undefined`, defaults, and partial update behavior. `enforced-by: typescript/eslint zod/require-strict`
- Pattern-keyed records, object intersections, and unknown fields. `enforced-by: typescript/eslint zod/require-strict`
- Real ULIDs, IPv6 addresses, and web hostnames accepted by integrations. `enforced-by: typescript/eslint zod/require-strict`
- Reserved property names, including `__proto__`, and the shape of returned validation errors. `enforced-by: typescript/eslint zod/require-strict`
...
Use documentation matching the installed release and enabled features. `enforced-by: typescript/eslint zod/require-strict`
