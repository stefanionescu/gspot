...
- A new dependency needs a reason in the commit: what it does that the platform and existing
  dependencies do not. `unenforced`
- Prefer a package that does one thing, is maintained, publishes types, and has no install scripts. `unenforced`
- One package manager per repository. One lockfile, committed, and installed with the frozen flag
  in CI and images. `enforced-by: integrity/manifest-policy`

## Versions

- Pin exact versions. No `^`, `~`, ranges, `latest`, or unpinned git references. `enforced-by: integrity/manifest-policy`
- The runtime version (Node, Python, Bun, Swift toolchain) is pinned once and every file that
  states it agrees. `enforced-by: integrity/manifest-policy`
- A package is installed only after the minimum release age the repository configures. A version
  published today does not enter the lockfile today. `enforced-by: integrity/install-policy`
- Upgrade one dependency per commit with the reason. Never upgrade broadly to satisfy a scanner
  without reading what changed. `unenforced`
...
- Remove a dependency the moment its last import goes. An unused dependency is a finding. `enforced-by: typescript/knip`
