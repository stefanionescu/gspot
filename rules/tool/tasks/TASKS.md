...
- Tasks are idempotent and safe to rerun. A destructive task (`db:reset`, `clean:all`) says so in
  its description and asks for confirmation on a terminal.
- Every task has a one-sentence description shown by the runner's list command.
