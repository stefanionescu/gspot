...
| `run/`        | `session.ts`, `plan.ts`, `execute.ts`, `check-command.ts`, `tool-runner.ts`, `command-parts.ts`, `parse-output.ts`, `broken-tool.ts`, `engines.ts`, `concurrency.ts`, `file-batches.ts`, `cache.ts`, `baselines.ts`, `baseline-command.ts`, `ignores.ts`, `fixers.ts`, `reproduce.ts`, `version-pin.ts`, `progress.ts`, `record/`     |
