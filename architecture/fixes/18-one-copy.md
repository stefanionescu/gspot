...
is written once over the syntax tree, with one set of limits, and the manifest of each language
lists it. This step also builds the checks the ledger promised and nobody wrote (K-248).
...
**Logic.** An analysis asks the language table for nodes and never names a language. For
TypeScript the structure engine runs the analysis, and the ESLint rules of the same idea leave
the plugin, so one id reports one fact in every language. A call-through replaces the
trivial-function check everywhere (K-102). A language gains an idea by adding its table row.

**What goes.** The three constant sets, the per-language copies, the plugin rules
`no-call-through`, `env-access-owner`, `import-layout`, `no-export-only-files`, and
`no-exported-alias-constants`, and `limits.sql.function_lines` as a key no check reads, which
gains its reader here.
