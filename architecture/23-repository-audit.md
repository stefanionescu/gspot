...
| `tests/harness/planted.ts` tolerates failed initialization when config exists and uses unchecked shell filesystem commands.                      | A test repository can pass after unsuccessful setup or fail to restore its intended state. Async `finally` handling is only a partial repair. | T-27, T-8                                                |
