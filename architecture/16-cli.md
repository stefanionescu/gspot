...
One binary, `gspot`, version 0.1.0 first. Fifteen commands. Each is a verb over
...
edited there; the one command that writes to it is `exceptions add`, because a
hand edit cannot validate an exception.
...
```text
gspot
  init          [--yes] [--rules-only | --no-rules | --import-rules]
                [--scope <name=path>] [--presets <ids>] [--declare <ext=kind>]
                [--runner mise|bun|npm] [--migrations none|all|<version>]
                [--strict | --baselines] [--gate hooks|ci|split]
  uninstall
  sync          [--check]
  upgrade       [--check] [--to <version>]
  install
  doctor
  check         [<id>] [--stage <name>] [--scope <name>] [--kind <kind>]
  ...
  config        <tool>
  explain       <rule>
  exceptions    add <check> | remove <check>
...
| `gspot init --scope` `--presets` `--declare` `--runner` `--migrations` `--strict` `--baselines` `--gate` | One flag per question. A flag answers its question; `--yes` answers the rest. With no terminal, a question no flag answers is exit 2. |
| `gspot init --rules-only` / `--no-rules` | All the rules and none of the checks, or the reverse. Whole choices, not partial ones. |
...
| `gspot sync` | Re-renders every generated file from `gspot.toml`. Idempotent. Run after any edit to `gspot.toml`. A preset added to an area with findings gets baselines the same way `init` writes them. A preset whose requirement is absent, or whose removal breaks another, is refused with the exact line to add or remove; `sync` never edits `gspot.toml`. |
| `gspot sync --check` | Fails when a generated file was hand-edited, a glob matches nothing, a referenced file is missing, a config has no reader, or a CI workflow still runs a tool gspot owns. Runs at pre-commit. |
...
| `gspot install` | Downloads every tool in `tools.lock` into `.gspot/bin/` and verifies each SHA-256, on every run, replacing anything that fails. Under the mise runner it writes the pins and mise downloads. gspot runs only the tools it installed; a tool on `PATH` is never used. The only command that touches the network by default. |
...
| `gspot check --kind types` | Every check of one kind. |
| `gspot check --since origin/main` | Only over files changed since a ref. The pull-request form. |
| `gspot check --skip <id>` | Skips one check for this run. Allowed only for ids in `skip_allowed` or in `gspot.local.toml`; refused otherwise; recorded in the run report either way. |
| `gspot check --unchecked` | Prints only the files nothing reads, with the reason each is unread. |
| `gspot fix` | Every fixer in a fixed order, formatters last, then the checks again to prove they converged. `gspot fix <id>` for one. |
...
| Command | Does |
| --- | --- |
| `gspot settings` | Every value resolved from `gspot.toml`, with its source: preset default, scope, root, or the file. Keys and values, nothing else. |
...
| Command | Does |
| --- | --- |
| `gspot exceptions add <check> [--rule <r>] [--paths <g>] [--finding <id>] --reason <text> --owner <name> [--expires <days>]` | Writes one `[[exception]]`. Refuses without `--reason` and `--owner`, defaults `--expires` to 90 days, prints the coverage the exception removes before writing. Bare, it turns the whole check off; narrowed, one rule, some paths or one finding. |
| `gspot exceptions remove <check>` | Deletes it. |

Everything else in `gspot.toml` is a hand edit followed by `gspot sync`. Every
table is documented in [06-settings.md](06-settings.md) with a copy-paste example.

## Hooks

| Command | Does |
| --- | --- |
| `gspot hooks install` | Writes `.githooks/pre-commit`, `pre-push` and `commit-msg`, each one line calling `gspot check --stage <name>`, and sets `core.hooksPath`. Refuses to overwrite a hook it did not write. `init` does this; this is for after a no at init or an uninstall. |
| `gspot hooks uninstall` | Removes the three files and unsets `core.hooksPath`. |

| Hook | Runs | Over |
| --- | --- | --- |
| `pre-commit` | every check that requires nothing, plus `sync --check` and `coverage --diff` | staged files, and the scopes whose policy changed |
| `pre-push` | the same, plus baseline and exception-limit assertions | the whole tree |
| `commit-msg` | commitlint | the message |

Nothing that needs a build, Docker or the network runs in a hook. Under
`gate = "split"` those run in CI; under `gate = "hooks"` the build checks run at
pre-push and the network checks only from `gspot check` by hand.
...
| 1 | The gate failed: a finding, a coverage gap, a hand-edited generated file, a missing tool, an exceeded or expired exception. The output says which. |
...
coverage     4127 paths   1 unchecked   0 partial   0 orphan
exceptions   7 / 12   3 expire within 90 days
