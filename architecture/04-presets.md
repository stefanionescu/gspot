...
`policy` is the kind for presets whose checks span languages. It is part of
...
A manifest holds every fact the CLI knows about its preset. The code names no preset, no
...
- `recommends` names presets that `init` selects with this one and a person can drop.
...
over that scope's files. Root-only presets (policy kind) run once over the whole tree.
...
The [enforcement ledger](06-enforcement-ledger.md#preset-enforcement-contracts) preserves
the detection, settings, and checks of each agreed preset. The v1 set is every preset the four reference repositories need. That covers a Python API, a Swift
...
- Name another preset in a template, or hold a default that a manifest owns.
...
At `recommended` the bash preset runs the checks that find a defect. They are strict
...
only add errors, as [04-presets.md](04-presets.md) builds it (K-74).
...
The rules of a library live in its own preset. No template names another
...
Render selected preset combinations and execute their pinned consumers. Verify
...
The i18n preset alone ships `i18n/locales` and its one setting. The nextjs preset
...
A planted Cloudflare repository without the security preset holds no
...
A preset that another preset recommends is selected only when its own `[detect]`
matches (presets without detection, such as naming, remain explicitly selectable). A manifest takes
