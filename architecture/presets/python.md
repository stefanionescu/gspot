...
uv. Installed through mise `pipx:`. gspot writes nothing into `pyproject.toml` (D-117). bandit, pip-audit and interrogate are not used: Ruff `S`, osv-scanner and Ruff
`D1` do their jobs.
...
| `python/basedpyright`                                                                                                                                          | commit | `basedpyright --baselinefile .gspot/<scope>/basedpyright-baseline.json --outputjson`, run in the scope, which holds the `pyrightconfig.json` stub                                  |
