...
Kind: concern. Requires: nothing. Dependency health: advisories, unused, duplicated,
...
| `integrity/dependency-ownership`   | commit                                                             | Python: `pyproject.toml` owns every dependency; no `requirements*.txt` unless a `[[generated]]` names it with `produced_by` (`uv export ...`), in which case `integrity/generated-fresh` checks it; no `pip install` outside `[tools.dependencies] pip_install_allowed`                                                                    |
