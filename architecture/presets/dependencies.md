...
| `integrity/dependency-ownership`   | commit                                                             | Python: `pyproject.toml` owns every dependency; no `requirements*.txt` unless a `[[generated]]` names it with `produced_by` (`uv export ...`), which the check then leaves alone; no `pip install` outside `[tools.dependencies] pip_install_allowed`                                                                                      |
