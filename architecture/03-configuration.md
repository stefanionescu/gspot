...
| --- | --- | --- | --- |
| `gspot.toml` | the repository | yes | The policy. Written once by `init`, appended by `gspot ignore`, edited by hand otherwise. |
...
- A key gspot does not know fails to load. There is no silent extra table.
...
- A baseline for a rule that no longer exists is reported by `sync --check` and deleted by hand.
