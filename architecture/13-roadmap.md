...
| Integrity                             | Done: `generated-drift`, `docs-headings`, `stale-paths`, `fences`, `readme-present`, `readme-shape`, `tsconfig-options`, `env-example`, `baselines-current`. Owed: `allowlists-resolve`, `suppressions`, `manifest-policy`, `lockfile-fresh`, `env-files`, `large-files`, `config-purity`, `task-policy`, `install-policy`. |
