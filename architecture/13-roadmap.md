...
| 2     | Hooks are added, not taken (A-3, D-101)                                                                               | a planted repository with `.githooks/pre-commit` runs its own hook and the gspot block on one commit, and `core.hooksPath` is unchanged; `uninstall` leaves the hook as it was    |
...
| 7     | Records and cache outside the repository (A-7, K-44, K-45, D-105)                                                     | a commit between `check` and `apply --lower-baselines` changes nothing; `.gspot/` in the app holds under 2 MB                                                                     |
| 8     | The policy text (A-8, A-9, K-51, D-106)                                                                               | the app policy holds no line over 120 characters and under 150 lines; one file in the repository is named `gspot.toml`                                                            |
...
| 10    | The trial form (D-107)                                                                                                | `init --trial` changes two paths; `init --finish` prints the plan of a full init                                                                                                  |
...
| 14    | Tests (T-1 to T-12, D-113)                                                                                            | a planted install with defaults, the mise runner and two scopes passes; `tests/config/` holds the shared values; the read of the folders listed as not read in 18-gaps.md is done |
| 15    | The app branch is redone from a release                                                                               | the branch holds no `file:` link, no `gspot.local.toml`, and its report is written from a run with no local skip                                                                  |
