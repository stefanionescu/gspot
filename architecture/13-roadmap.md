...
| Naming engine                         | Done: bash, TypeScript, JavaScript, SQL, and Swift extractors, paths, the policy schema check. The Python extractor is not built; `python.wasm` is embedded and unused.                                                                                                                                                                                                                                                                                                            |
| Structure engine                      | Done for directories and shell, with a planted defect for every check. Swift and Python analyses are not started; SQL has `sql/block-comments` and `sql/file-length`.                                                                                                                                                                                                                                                                                                              |
...
| Self-lint                             | Done: `gspot check` passes 131 checks with no `[[ignore]]` (D-27), with `secrets`, `security`, `dependencies`, `licenses`, and `duplication` selected. Kept green from here on.                                                                                                                                                                                                                                                                                                    |
...
| Phases 2 to 5, Phase 7                | Phase 5 done: `secrets`, `security`, `dependencies`, `licenses`, `duplication`. Phase 2 done except Python: `sql`, `postgres`, `supabase`, `docker`, `nginx`, `express`, `vitest`, `ansible`. Phase 4 done: `swift`, `xcode`, `xctest`; the Swift structure analyses are owed. Not started: `python`, `pytest`, `fastapi`, Phase 3 (the web presets), Phase 7. Owed across phases: the manual rewrite (G-10) and the deeper unit tests of apply, carry, plan, execute, and doctor. |
