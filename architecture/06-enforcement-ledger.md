...
| `docs.sentence_words` | 25 | SA `LINTING.md` | Vale `gspot.sentence-length` |
| `docs.list_item_words` | 20 | same | Vale `gspot.step-length` |
...
All port into `eslint-plugin-gspot` with their tests. Source files are the three `plugin/rules/`
...
| no-support-in-dirs | SA | `gspot/no-support-in-dirs` |
| no-tests-support-imports | SA | `gspot/no-tests-support-imports` |
...
| `process.env` only in the configuration owner | SS `no-client-environment` generalised | `gspot/env-access-owner` with `[architecture] roles.env` |
...
| security | the 11 rules on in `SA rulesets/security.js`; `detect-object-injection` and `detect-non-literal-fs-filename` off | SA, LA, SS |
...
| restricted imports | `next/image` in favour of the project picture component (a `[tools.eslint]` slot, off by default) | SS |
| i18next | `no-literal-string` for JSX text in localised apps | SS dependency |
...
| placeholder docstrings (`Handle `, `Provide `) | `functions/python.py` | `structure/placeholder-docstring` |
...
| every folder allowlist entry resolves | `integrity/folder.py` | `integrity/allowlists-resolve` |
...
| duplicate function bodies, normalised, min lines | TI, SA, LA | `structure/duplicate-functions` |
...
| interpreter policy: `#!/usr/bin/env bash` or `#!/bin/bash`; line 2 is `#`; line 3 a concrete description; line 4 `# Runtime: Bash N.N+, macOS and Linux.`; Bash 4 features (`mapfile`, `declare -A`, case conversion, `coproc`, `wait -n`) named as such; computed directory constants use `CDPATH=`, `cd --`, `pwd -P` and a failure path; `main "$@"` last in executables; readonly top-level assignments; library files declarative at top level; executable bit matches the file's role; beyond the reference set: `set -euo pipefail` and `shopt -s inherit_errexit` in every executable, `trap` cleanup for every temporary file | TI `bash.py` | `structure/shell-interpreter` |
...
| Rule | gspot |
| --- | --- |
| SwiftLint: the 87 opt-in rules, 4 analyzer rules, limits, `missing_docs` on open and public, `unused_import` keep list; `identifier_name` and `type_name` off in favour of the naming engine; `no_magic_numbers`, `type_contents_order`, `one_declaration_per_file`, `file_types_order`, `no_empty_block` on with a baseline; beyond the reference set `explicit_acl`, `explicit_top_level_acl`, `private_over_fileprivate`, `file_name_no_space`, `file_header` | `swift/swiftlint`, `swift/swiftlint-analyze` (push, needs build) |
...
| commitlint conventional, type enum, scope enum from `[tools.commitlint] scopes`, lower-case subject, header 120, body line 200 | all | `commits/commitlint` (commit-msg hook) |
...
| folder allowlists resolve | SA, LA, TI | `integrity/allowlists-resolve` |
...
| CodeQL false-positive file names an existing path | SA, LA | `integrity/allowlists-resolve` |
...
The 49 files of `reference-rules/merged/` land as the corpus, per layer, in
[09-rules-corpus.md](09-rules-corpus.md). `CLAUDE.md` and `AGENTS.md` gain a managed block that
...
gspot does not use (lizard 1.21.0, madge 8.0.0, sort-package-json 4.0.0, bearer 2.0.1,
bandit, pip-audit, interrogate, pyright) are listed in [11-toolchain.md](11-toolchain.md) with
the tool that does their job.
