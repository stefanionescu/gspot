...
| no-trivial-functions                                                                                                                                                                                                                                              | SA, LA                                                                  | `gspot/no-trivial-functions`                                                                                                                                            |
...
| Semgrep: no `torch.jit.script`, no `torch.load`, no `LSTMCell`, no direct diskcache, no HF token literal, no obsolete markers                                                                                                                                                                                                                   | `config/security/semgrep/*.yml`                                | `security/semgrep` with `presets/python/semgrep/python.yml`; the rules that name one project go to that project (K-248, K-218)                                                                             |
...
| Semgrep iOS: 14 rules (keychain accessibility, secrets in plist and UserDefaults, insecure HTTP, ATS exceptions, weak hashes, UIWebView, unsafe pointer casts, sensitive logging, script eval and unquoted vars, JavaScript in WKWebView, hardcoded keys and credential URLs)                                                                                                                                                                         | `security/semgrep` with `presets/swift/semgrep/ios.yml` (K-248)                                        |
...
| trivial PL/pgSQL functions: a function whose body only calls one other function with its parameters, or has at most the trivial statement ceiling (`SA functions/sql.js`)                                                 | `structure/trivial-function`, `structure/trivial-function` for sql |
...
and abstraction preferences remain available at `all`; recommended requires a demonstrated defect.
...
The rule skips the entry files of `tools.knip.entry`, the one list of entries gspot
keeps.

The rule takes an `entryFiles` option, and the template passes the setting.

The rule test moves `main.ts` to the valid cases (T-20).
...
| `swift/trivial-function`, `swift/trivial-function`, `swift/duplicate-functions` | commit      | analyses on the Swift grammar                                                                                           |
...
The structure checks read the Swift grammar and carry the language in their names:
`swift/trivial-function`, `swift/trivial-function`, `swift/duplicate-functions`,
