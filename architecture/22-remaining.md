...
- [ ] D-129: Delete `gspot declare`, `gspot profile check`, six of the seven lists of `gspot allow`, `doctor --offline`, `uninstall --keep-hooks`, `apply --check`, and `--dry-run` on `ignore` and `set`. File explanations are implemented by `explain <path>`.
