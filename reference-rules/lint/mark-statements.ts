...
for (const file of walk(CORPUS)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    let line = lines[i];
    if (i === 0 && line === "---") { inFrontMatter = true; out.push(line); continue; }
    if (inFrontMatter) { out.push(line); if (line === "---") inFrontMatter = false; continue; }
    if (/^\s*`{3,}/.test(line)) { inFence = !inFence; out.push(line); continue; }
    if (inFence) { out.push(line); continue; }
    const isList = /^\s*(?:[-*]|\d+\.)\s+\S/.test(line) && !/^\s*[-*]\s+\[/.test(line);
    const isTable = line.trim().startsWith("|");
    const isHeading = /^#{1,6} /.test(line);
    const prevBlank = i === 0 || lines[i - 1].trim() === "";
    const isParagraphStart = prevBlank && !isList && !isTable && !isHeading && line.trim() !== "" && !/^\s{2,}/.test(line) && !/^(Bad|Good|Avoid|Use|Rules|Example|Examples|Good [^:]*|Bad [^:]*)[:]$/.test(line.trim());
    const isStatement = isList || (isParagraphStart && IMPERATIVE.test(line.trim()));
    if (!isStatement) { out.push(line); continue; }
    // An item or sentence that ends with a colon introduces a list or a block; the rule is what follows.
    { let last = i; while (last + 1 < lines.length && /^\s{2,}\S/.test(lines[last + 1]) && !/^\s*(?:[-*]|\d+\.)\s/.test(lines[last + 1])) last++; if (/:\s*(`(?:enforced-by: [^`]+|unenforced)`)?\s*$/.test(lines[last])) { for (let k = i; k <= last; k++) out.push(lines[k].replace(MARKER, "")); i = last; continue; } }
