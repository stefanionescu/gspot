...
- Do not leave a suppression whose cause is gone. A suppression that suppresses nothing is a
  finding of its own.
- There is no ticket field. A field nobody fills honestly is worse than no field.

Every language has its own syntax for this and the rule is the same in all of them: the ESLint
