...
Use App Router. Update affected routes and callers together; do not maintain a parallel Pages `enforced-by: integrity/route-segments`
...
- Check the package manifest and lockfile for the installed Next.js and React versions. Identify the
  ...
  TypeScript, ESLint, styling, data, and internationalization setup. Use the installed APIs. `enforced-by: integrity/manifest-policy`
- Find the existing components, schemas, queries, actions, design tokens, and state that support the
  ...
  behavior. Add folders when they clarify a responsibility in the application. `enforced-by: security/semgrep`
...
- Use route files for routing and assembling the page. Keep a small page in `page.tsx` when that is
  clear. Extract a component or module when it has its own UI, data, or domain responsibility. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Follow Next.js special filenames and exports. A `page` exposes UI; a `route` exposes an HTTP
  endpoint. Put them in different route segments. `enforced-by: integrity/route-segments`
...
- Keep route-specific composition beside its route. Private `_components` and `_lib` folders are
  useful when they make that locality explicit. `unenforced`
- Keep domain operations with their feature when both UI and server behavior represent that feature.
  ...
  them through a barrel. `enforced-by: typescript/eslint boundaries/element-types`
- Move a component into shared UI when multiple real consumers use the same presentation/interaction
  ...
  arbitrary domain entities internally. `unenforced`
- Keep an integration's credentials, transport setup, and runtime selection at its integration
  ...
  inconvenient existing API. `enforced-by: typescript/eslint boundaries/element-types`
- Colocate a schema with the contract it validates. A browser-safe input schema must not import the
  privileged implementation of the operation it describes. `unenforced`
- Keep ordinary local props and helper types next to their implementation. Generated database types
  and shared external contracts have distinct owners. `enforced-by: typescript/eslint boundaries/element-types`
- Import the narrow module that owns a symbol. Do not depend on a root barrel whose exports span
  database access, React hooks, UI, and runtime configuration. `enforced-by: typescript/eslint boundaries/element-types`
- When a move is required, update all callers, import aliases, and framework references.
  ...
  needed follow-up. Delete superseded code instead of keeping a second path. `unenforced`
...
- Keep route files focused on framework input, metadata, server composition, and the feature entry.
  Do not put the whole interactive screen into `page.tsx`. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Use layouts to arrange sections and control screen geometry. Put queries, editor callbacks, and
  row rendering in the sections or components that use them. `enforced-by: typescript/eslint boundaries/element-types`
- Keep a section's data loading, pending state, and error handling together. Keep unrelated sections
  usable while that section loads or recovers from a failure. `enforced-by: typescript/eslint boundaries/element-types`
...
- Import downward through the hierarchy. A component does not import its section, and a section does
  ...
  same behavior. `enforced-by: typescript/eslint boundaries/element-types`
- Create a section because it owns a meaningful responsibility. Do not add empty layout/section
  wrappers solely to reproduce every folder in the convention. `enforced-by: typescript/eslint boundaries/element-types`
...
Place loading and error handling around the section doing the work. Keep the page layout, `enforced-by: typescript/eslint boundaries/element-types`
...
- Place the Suspense/query loading boundary around the section that waits. Do not await its work
  ...
  already-awaited work. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Keep section skeletons alongside their section or feature. Match the important geometry: header
  height, row/media space, and the scroll viewport's constraints. `enforced-by: typescript/eslint boundaries/element-types`
- Distinguish the first load from background refresh and next-page loading. Keep usable content
  mounted during refresh and page-fetch failure. `unenforced`
- Scope retry to the failed operation. Retrying a sidebar request never resets a draft in an
  unrelated editor or discard a history scroll anchor. `enforced-by: typescript/eslint boundaries/element-types`
- Give each failure a recovery action that can resolve it. Preserve the original error in safe
  server diagnostics and show a translated error message in the UI. `enforced-by: typescript/eslint boundaries/element-types`
- Keep route-level error boundaries for failures that escape section recovery. Verify root/global
  error UI separately because its usual providers may be absent. `enforced-by: typescript/eslint gspot/require-server-only`
- When switching resources, reset the section's identity and pending state deliberately. Do not
  allow an old request to populate a newly selected section. `enforced-by: typescript/eslint boundaries/element-types`

### Stream independent sections

Use streaming to reveal useful content while independent work completes. On an initial App Router `enforced-by: integrity/route-segments`
...
- Use `loading.tsx` for the segment's page and descendants. Its boundary sits inside the segment's
  ...
  consumer when the layout contains an independently loading section. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Use sibling `<Suspense>` boundaries for sections that can become useful independently. Use nested
  ...
  task rather than wrapping every component. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Move request-dependent reads into the component that needs them. Pass `params`, `searchParams`, or
  ...
  Start independent work early without moving protected reads ahead of authorization. `enforced-by: security/semgrep`
- With Cache Components, inspect which work can enter the prerendered shell. An async function is
  ...
  Handle uncached reads, runtime inputs, and nondeterministic output according to the active model. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
...
- Keep important headings and likely largest-content elements in early content when their data is
  ...
  revealed if its boundary has not resolved. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Reserve realistic space for resolved content and keep fallbacks localized and accessible. Measure
  layout shifts and interaction responsiveness as well as time to the first response byte. `enforced-by: typescript/eslint boundaries/element-types`
- Let React manage streamed DOM replacement and hydration. Keep application code independent of
  private payload markers, inline swap scripts, and generated boundary IDs. `enforced-by: typescript/eslint gspot/require-server-only`
...
Choose the response status, headers, and cookies before the response is committed. After streaming `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
...
- Place error recovery at the intended section or route boundary. A route-level `error.tsx` can
  ...
  alone disappears. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Keep authorization at the protected operation even when an early proxy redirect helps navigation.
  Do not send private content while waiting to decide whether the caller may receive it. `enforced-by: security/semgrep`
- Verify metadata for ordinary browsers, DOM-capable crawlers, and HTML-limited crawlers relevant to
  ...
  release. Avoid assuming every crawler receives one fully buffered document. `enforced-by: integrity/manifest-policy`
- Keep shell dependencies available when the route renders at request time. Check crawler requests
  and cold renders when normal visitors receive a prerendered shell. `unenforced`
...
Use each framework file for its defined purpose. Next.js chooses when to render or call these files `unenforced`
...
Keep the surrounding page usable while a section loads. Loading another table page replaces `enforced-by: typescript/eslint boundaries/element-types`
...
Choose a client boundary by identifying which code needs to run in the browser and what data it `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
...
1. Read data in the responsible server module. Select the fields the user is allowed to see and the
   browser needs. This selected response is the data transfer object (DTO). `unenforced`
2. Identify the behavior that needs event handlers, browser APIs, or state. `unenforced`
3. Keep the surrounding server-rendered layout outside that interactive entry. `enforced-by: typescript/eslint boundaries/element-types`
...
5. Trace its imports, including shared helpers and re-exports. Browser reachability can grow through
   a helper even when the page itself remains a Server Component. `enforced-by: typescript/eslint gspot/require-server-only`
6. Verify direct load, hydration, and client navigation. Each path displays the expected
   content and support the same interactions. `enforced-by: typescript/eslint gspot/require-server-only`

Use `'use server'` for Server Functions. Follow the framework's rules for their callable exports, `enforced-by: typescript/eslint gspot/require-server-only`
...
- Use version-appropriate request APIs. Next 15 introduced async request APIs; Next 16 requires
  ...
  their types before standalone typechecking. `enforced-by: integrity/manifest-policy`
- Use `next/link` for internal navigation, the App Router navigation APIs in App Router, and
  ...
  redirect API. `enforced-by: integrity/route-segments`
- Model pending, empty, error, and success states. Missing resources use the framework's not-found
  ...
  Do not swallow framework redirect/not-found control flow in broad catches. `enforced-by: security/semgrep`
- Metadata, canonical URLs, social previews, robots, sitemap, and public content availability match
  ...
  API rather than competing ad hoc head managers in App Router. `enforced-by: integrity/route-segments`
...
- Change the code needed for the requested outcome. Refactor only where the implementation requires
  it; report unrelated violations instead of expanding the task into general cleanup. `unenforced`
- Do not disable existing checks to hide failures. `enforced-by: integrity/required-rules`
- Choose runtime and deployment features for the application. Do not introduce Edge, PWA,
  analytics, or third-party providers without a concrete requirement. `enforced-by: typescript/eslint gspot/require-server-only`
...
- Choose one server data owner per domain. Pages, tRPC, Route Handlers, and Server Actions call the
  ...
  in each transport. `enforced-by: typescript/eslint gspot/require-server-only`
...
- Pending UI prevents accidental duplicate submissions and communicates progress. Validation errors
  ...
  Writes must not occur during rendering, GET requests, or route prefetching. `enforced-by: typescript/eslint boundaries/element-types`
...
- Determine whether Cache Components is enabled before selecting APIs. Do not mix recipes from
  ...
  changed in Next 15. `enforced-by: integrity/route-segments`
...
- Clear private browser caches on logout or identity changes. Service workers must not cache
  ...
  and a new session. `enforced-by: security/semgrep`
...
- Use supported custom values when named profiles do not match the feature. Check their ordering
  constraints and units. Keep one effective `cacheLife` call per cached function invocation. `unenforced`
- Revalidation is demand-driven; a duration is not a scheduled background job. Check behavior after
  inactivity as well as under frequent requests. `unenforced`
- Check how short lifetimes affect prerendering and prefetch eligibility in the installed release. A
  cache directive does not guarantee that its content joins the static shell. `enforced-by: integrity/manifest-policy`

See [cacheLife](https://nextjs.org/docs/app/api-reference/functions/cacheLife).

Choose a data-level cache for a reusable read and a UI-level cache for reusable rendered output. `unenforced`
...
- Account for arguments and captured values in cache identity. Keep request reads outside ordinary
  shared cache scopes and pass validated, nonsecret inputs to reusable reads. `enforced-by: security/semgrep`
- For per-request random output or time-dependent synchronous reads, use supported request-time APIs
  ...
  intended behavior. Synchronous database or filesystem access is not necessarily immutable data. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Use `use cache: remote` only with a supported handler and a measured need for shared storage.
  ...
  survival aligned with deployment and build identity. `unenforced`
- Use `use cache: private` only when the installed release supports the required behavior. Review
  ...
  authorization or make sensitive serialized data safe automatically. `enforced-by: integrity/manifest-policy`
- Review metadata and viewport functions under the same caching model as the page. Keep their
  runtime reads and freshness requirements consistent with the content they describe. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`

See [Cache Components](https://nextjs.org/docs/app/getting-started/caching).

### Choose the revalidation operation

Choose revalidation from the required freshness and the server entry point performing the write. `unenforced`
...
- Attach `cacheTag` inside the cache scope that produces the reusable result. A tag groups entries
  for invalidation; it does not substitute for a complete cache key or an access check. `unenforced`
- Treat `revalidateTag` as marking entries for subsequent reads, not eagerly rebuilding every
  ...
  deprecated. [revalidateTag](https://nextjs.org/docs/app/api-reference/functions/revalidateTag). `unenforced`
- Keep `updateTag` in Server Actions. A tRPC mutation or API handler does not become a Server Action
  ...
  [updateTag](https://nextjs.org/docs/app/api-reference/functions/updateTag). `enforced-by: typescript/eslint gspot/require-server-only`
- Scope `revalidatePath` deliberately. A dynamic route pattern needs the appropriate `page` or
  ...
  occurs on a later visit; it does not push fresh UI into all open browsers. `enforced-by: integrity/route-segments`
- Invalidate shared data tags when other routes depend on the same changed resource. Invalidating
  ...
  [revalidatePath](https://nextjs.org/docs/app/api-reference/functions/revalidatePath). `unenforced`
- Authenticate revalidation webhooks, validate their payloads, and derive allowed tags or paths from
  the verified event. Keep arbitrary caller-supplied invalidation targets out of the endpoint. `enforced-by: security/semgrep`
- Reconcile TanStack Query, Drizzle caches, and other stored copies separately where applicable.
  Refreshing a browser route alone can reuse unchanged server data. `unenforced`

### Verify cached navigation and prefetching

Check direct visits, client transitions, refresh, and Back/Forward separately. A working initial `unenforced`
...
- Verify known and unknown dynamic parameters. Use `generateStaticParams` for intended prerendered
  values, and check the installed model's behavior for other values and missing resources. `enforced-by: integrity/manifest-policy`
- Inspect the actual prefetched content before forcing `prefetch={true}`. With supported Partial
  ...
  Keep prefetch cost bounded on large lists and frequently changing search results. `unenforced`
- Keep prefetching read-only and authorized. Prefetched private data is already browser-accessible;
  hiding the destination's UI does not protect it. `enforced-by: security/semgrep`
- Verify which fallback appears before a navigation completes, including with a cold cache or a
  ...
  always omitted from prefetching. `enforced-by: typescript/eslint @typescript-eslint/no-floating-promises`
- Exercise changed permissions, locale, query parameters, and session identity against prefetched
  results. Confirm invalidation reaches the stored copy used by the next navigation. `enforced-by: security/semgrep`
...
Include every input that changes the result in the cache key. For a public list, these inputs can `unenforced`
...
- Can two users with different permissions request the same apparent resource ID? `unenforced`
- Can the same ID exist under different tenants or projects? `enforced-by: security/semgrep`
- Does the returned shape or text vary by locale or user preference? `unenforced`
- Can a role change, permission revoke, or deletion make the cached result unsafe? `unenforced`
- Which writes change this value, and which aggregate/list/detail views depend on it? `unenforced`
- Where is each cached copy stored and how is it invalidated on every instance? `unenforced`
- What happens immediately after the initiating user saves a change? `unenforced`

Make these choices visible in the cached function's inputs and invalidation code. Use tags to mark `unenforced`
...
Do not collapse all read states into `data ? content : spinner`. That condition cannot distinguish a `unenforced`
...
Choose the transport from the consumer and behavior, not from a desire to use every Next.js API. A `unenforced`
...
Keep Route Handlers focused on five steps: extract input, validate it, establish request context, `enforced-by: integrity/route-segments`
...
Use documentation matching the installed release and enabled features. `enforced-by: integrity/manifest-policy`
