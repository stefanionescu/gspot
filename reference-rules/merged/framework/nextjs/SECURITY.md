...
- Verify authorization and data isolation with negative integration cases and focused boundary
  review. Exercise the deployed application's access controls and runtime configuration. `unenforced`

### Keep configuration out of browser bundles

Keep private environment values out of `next.config.*`'s `env` option. That option makes configured `unenforced`
...
- Inspect the composed configuration, including wrappers, imported fragments, and deployment
  substitutions. A browser-only environment lint rule cannot inspect build-time injection by itself. `unenforced`
- Verify the browser output of a production build with harmless marker values when changing secret
  ...
  delivered asset, removing the source reference alone does not revoke that credential. `unenforced`
- Keep type and lint failures visible. Do not enable `ignoreBuildErrors` or a version-specific
  `ignoreDuringBuilds` escape hatch to make delivery pass. `unenforced`
- Align Next, its ESLint configuration, React, the deployment adapter, and bundler plugins with the
  ...
  same APIs or adapter work with a newer Next release. `unenforced`
...
Use verified identity consistently across data clients. A server admin/service-role client can `unenforced`
...
- Cover links, image sources, inline event attributes, embedded frames, SVG, and dangerous URL
  ...
  to make markup safe. `unenforced`
- Restrict `dangerouslySetInnerHTML` to a reviewed rendering adapter. The shared preset's
  ...
  sanitizer or the exact trusted static source. Keep sanitization tests beside that adapter. `unenforced`
- Apply the same policy to streaming partial messages and final persisted messages. If parsing is
  asynchronous, prevent an older parse from overwriting the latest text or a different message. `unenforced`
- Initialize Tiptap in a Client Component with the installed version's supported SSR behavior,
  ...
  [Tiptap's Next.js integration](https://tiptap.dev/docs/editor/getting-started/install/nextjs). `unenforced`
- Keep editor updates as editor transactions. Avoid serializing the full document, rewriting HTML,
  ...
  composition. Use supported decorations, marks, or targeted transactions for visual annotations. `unenforced`
- Do not submit while an IME composition is active. Test Enter, Shift+Enter, mobile keyboards,
  multiline paste at the current selection, undo/redo, and the empty-to-nonempty transition. `unenforced`
- Define the saved representation and its size limits. Editor JSON, HTML, and plain text have
  different contracts; an editor extension schema does not make arbitrary saved HTML trusted. `unenforced`

### Scope service workers and private media

Treat the browser's service-worker cache as an additional cache owner. HTTP headers and query-cache `unenforced`
...
- Cache only the intended public assets. Exclude authenticated API responses, chat content,
  ...
  offline feature defines storage, expiry, logout, and account-switch behavior. `unenforced`
- Match worker routes against actual request URLs, methods, origins, and paths. A broad image-file
  suffix match can include authenticated media; a pathname regex may not match an absolute URL. `unenforced`
- Define cache versioning and worker activation behavior. Test an existing controlled tab during an
  ...
  discard an unsaved draft or serve another account's content. `unenforced`
- For object storage, authorize the bucket and object path, not just the existence of a signed-in
  ...
  variant. Enforce resource access in database and storage policies. `unenforced`
- Treat signed URLs as bearer access with an expiry. Keep them out of telemetry and persistent
  shared caches. Bound refresh on expiry and clear account-scoped media/query state at logout. `unenforced`
- Prefer an authorized media delivery mechanism appropriate to the runtime over repeatedly moving
  ...
  interrupted loads, and memory use with several visible videos. `unenforced`
- Clean up image/video listeners, preload work, and object URLs. When `src` changes, ensure a late
  load event cannot mark the new source as loaded or keep a failed preview from blocking recovery. `unenforced`

### Separate telemetry and provider results

Use one initialization owner for browser analytics. Avoid loading both an inline bootstrap and an `unenforced`
...
- Define allowed event properties. Mask or exclude chat transcripts, editor content, auth inputs,
  ...
  actual captured payloads, including custom controls and contenteditable elements. `unenforced`
- Reset analytics identity on logout and account switch. Keep anonymous-to-authenticated identity
  changes explicit; a module's prior identity must not label a different user's events. `unenforced`
- Telemetry failure must not replace an operation's intended response, and diagnostic code must not
  ...
  supported lifecycle for deferred work. `unenforced`
...
- Propagate an abort signal to upstream work when the provider/runtime supports it. `unenforced`
...
- Once headers are committed, report late failures through the defined stream protocol or close the
  ...
  completed. Follow the existing cancellation and persistence rules above. `unenforced`
...
Treat an inference proxy, retrieval request, and usage-verification endpoint as separate callable `unenforced`
...
- Derive resource ownership and allowed model and provider choices on the server. Bound input
  ...
  Browser token estimates and disabled buttons are feedback, not quota enforcement. `unenforced`
- Keep private prompts and retrieved passages out of query strings, referrers, and analytics URLs.
  Use a bounded request body for sensitive or costly generation operations. `unenforced`
- Treat retrieved text and generated output as untrusted data. They cannot authorize a tool call,
  ...
  results before using them as identifiers or operational instructions. `unenforced`
- Give generation and durable persistence a clear owner. If generation succeeds but saving fails,
  ...
  Define how cancellation and provider errors affect partial output and reserved quota. `unenforced`
- Use shared quota and concurrency state when several instances serve requests. Test direct
  ...
  and concurrent submissions. `unenforced`
...
- Check the host's response-streaming support and execution limits. Static export cannot perform
  ...
  them; `X-Accel-Buffering: no` is not a universal CDN control. `unenforced`
- Check compression flushing and client buffering. Compare compressed delivery with a diagnostic
  ...
  decisions based on measured user experience. `unenforced`
- Record request start, response headers, body-read timestamps, and completion. Starting the timer
  only after `fetch()` resolves hides the time spent waiting for headers. `unenforced`
- Inspect progressive visible content as well as timed body reads. An early first byte followed by a
  long download can have other causes and does not by itself prove sections streamed usefully. `unenforced`
- Treat network chunk boundaries as transport details. They need not match Suspense boundaries or
  ...
  `Transfer-Encoding: chunked` header. `unenforced`
- Verify slow data, slow networks, cancellation, mid-stream errors, and representative crawler
  requests. Check that the final page has usable content and the expected metadata and status. `unenforced`
- Keep diagnostics independent of private React payload syntax. Use observable timing, section
  visibility, accessibility, and final outcomes as evidence. `unenforced`
