...
- Define supported locales, the default locale, and locale selection in one place. Use BCP 47
  ...
  supported list before loading messages. Build catalog import paths from validated values only. `enforced-by: integrity/locales`
- Choose routing deliberately. Locale-prefixed public sites need locale-aware links, redirects,
  ...
  is valid for applications that do not need separately indexed locale URLs.
  `enforced-by: integrity/locales`
- Define precedence among URL, saved preference, negotiated header, and default; do not create
  redirect loops. `enforced-by: integrity/locales`
- Use the established i18n library of the framework. Translate on the server where rendering
  happens on the server; pass translated labels or scoped messages to the client. Do not ship the
  entire catalog to every client boundary because a provider supports it. `enforced-by: integrity/locales`
- Translate all application-authored user-facing content: headings, buttons, empty/error/loading
  ...
  have separate contracts; do not blindly translate them. `enforced-by: integrity/locales`
- Use stable semantic keys and complete messages. Do not concatenate translated fragments, pluralize
  ...
  callbacks trusted. `enforced-by: integrity/locales`
- Catalogs agree on nested leaf keys, variable roles, and rich-text tags. ICU must parse and include
  ...
  call sites. `enforced-by: integrity/locales`
- Format numbers, currencies, dates, relative time, and time zones through locale-aware APIs. Do not
  ...
  timestamps. `enforced-by: integrity/locales`
- Set the document's `lang` and `dir` for the selected locale. Use logical spacing and alignment
  properties so they adapt to text direction. Test supported RTL locales.
  `enforced-by: integrity/locales`
- Mirror an icon or image only when its meaning requires it. Check that long translations and
  different writing systems fit controls, and remeasure virtual rows while preserving the anchor.
  `enforced-by: integrity/locales`
- Language changes update server-rendered content, client labels, metadata where relevant, and
  ...
  through a validated write boundary. Do not show a successful selection while the request failed. `enforced-by: integrity/locales`
- Localized data is partitioned correctly in server, browser, and CDN caches. Never store a user's
  locale in mutable server module state. A request-local i18n configuration owns resolution. `enforced-by: integrity/locales`
- Make missing-key behavior visible during development. Any production
  ...
  locales get the documented default or not-found behavior, not an arbitrary dynamic import failure. `enforced-by: integrity/locales`
- Verify default and another supported locale, long text, plural cases, rich text, form failures,
  ...
  needs it. `enforced-by: integrity/locales`

Validate catalog key equality, nonempty string leaves, ICU syntax, variables, select options, and `enforced-by: integrity/locales`
...
Translation keys describe stable product concepts. Their names let a reader find the owning
...
- Group related messages under a consistent namespace. Share a message only when its meaning and
  grammatical role are the same everywhere it is used. `enforced-by: integrity/locales`
- Pass complete sentences or clauses to translators. Word order and pluralization vary; assembling
  fragments in code prevents correct translation. `enforced-by: integrity/locales`
- Treat placeholder names as part of the message contract. Changing a placeholder requires changing
  all callers and catalogs together. `enforced-by: integrity/locales`
- Use explicit plural/select constructs for grammatical variants. Locale-specific plural categories
  can differ while still requiring the same input value. `enforced-by: integrity/locales`
- Keep semantic rich tags stable and map them to trusted UI components. Do not accept arbitrary
  executable markup from a translator or provider response. `enforced-by: integrity/locales`
- Translate safe validation/error codes at the presentation owner. A raw schema exception, SQL
  message, or provider error is not a user-facing translation key. `enforced-by: integrity/locales`
- Keep nontranslatable values such as user-authored names and resource IDs as interpolation data
  rather than trying to turn them into message identifiers. `enforced-by: integrity/locales`
...
Choose which locale source takes precedence before adding middleware or proxy logic. A public site `enforced-by: integrity/locales`
...
Validate the candidate locale against a fixed supported set before selecting a catalog. Negotiation `enforced-by: integrity/locales`
...
Translate server-rendered content on the server. Give each client provider the messages and settings `enforced-by: integrity/locales`
...
Use documentation matching the installed release and enabled features. `enforced-by: integrity/locales`

| Topic                | Primary source                                                                          |
| -------------------- | --------------------------------------------------------------------------------------- |
| ICU MessageFormat    | [ICU MessageFormat](https://unicode-org.github.io/icu/userguide/format_parse/messages/) |
