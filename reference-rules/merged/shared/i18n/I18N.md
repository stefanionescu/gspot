...
- Choose routing deliberately. Locale-prefixed public sites need locale-aware links, redirects,
  ...
  URL, saved preference, negotiated header, and default; do not create redirect loops. `enforced-by: integrity/locales`
...
- Set the document's `lang` and `dir` for the selected locale. Use logical spacing and alignment
  ...
  controls, and remeasure virtual rows while preserving the anchor. `enforced-by: integrity/locales`
