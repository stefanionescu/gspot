...
- Source roots are named for what they own: content (authored bodies), templates (page-owned
  markup and route metadata), assets (source media), site configuration (data only), build (the
  generator and asset pipeline), edge functions (middleware only), browser scripts. `enforced-by: typescript/eslint boundaries/element-types`
