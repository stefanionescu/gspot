...
Read the manifest and lockfile before choosing a driver, schema API, or migration command. Record `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Select a release deliberately. A documentation command containing `@rc` is not a reason to move an
  application to a release candidate. Review compatibility and migration changes before upgrading. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Check the installed exports and types for column casing, relations, RLS, and configuration. APIs
  ...
  differ between releases. Use the API supported by the installed version throughout the operation. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Choose the Drizzle adapter that matches the actual driver. A `postgres` client uses the
  ...
  transaction support before selecting an HTTP driver or Edge runtime. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep generated migrations, snapshots, and the journal in the format expected by the selected
  runner. Update them together during a Kit upgrade. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use locally installed executables through the project's task runner. Hooks use those tools and
  do not fetch a new CLI version while checking a change. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

## Configure connections

Keep connection construction in a server module with a clear lifetime. In Next.js, mark the runtime `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Validate the connection URL before creating the driver. Report a missing variable by name without `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Verify the endpoint and network support in the project's connection settings. Use a separately `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- With Postgres.js and Supabase transaction pooling, set `prepare: false`. This disables prepared
  ...
  changes. [Drizzle with Supabase](https://orm.drizzle.team/docs/connect-supabase). `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Set connection, idle, and query timeouts for the workload. Budget connections across all running
  instances, background workers, and deployments. A pool limit on one process is not a global limit. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Reuse the process's bounded pool when the runtime permits it. Avoid creating a pool per request or
  multiplying pools during development reloads. Keep request identity out of reusable client state. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Close short-lived script connections in their cleanup path. Do not close a shared application pool
  after each query. Ensure failed scripts release their connections and return a failure code. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep certificate verification enabled for encrypted remote connections. Fix trust configuration
  instead of disabling verification to make a connection succeed. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Transaction pooling can move successive transactions between server connections. Keep settings
  ...
  operations that require session continuity, such as a long-lived notification listener. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

## Organize database code

Separate schema declarations, driver construction, domain operations, and migration execution. Place `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Keep a small schema together. Split a larger schema by domain or related tables when that makes
  ...
  every managed table, enum, sequence, view, and policy it needs. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Inspect discovery after moving files. A missed schema export can make generation propose a drop;
  an unintended export can make it propose creating a provider-owned object. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep schema modules free of connection creation, requests, environment loading, and migration
  execution. Kit needs to evaluate declarations without starting the application. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Put each domain query beside the behavior it serves. Name operations for their purpose rather than
  collecting unrelated work in global `insert`, `select`, `update`, and `delete` files. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Pass a database or transaction handle to operations that participate in a larger transaction. Keep
  the handle typed for the selected adapter and supported operations. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Infer stored-row and insert types with `$inferSelect` and `$inferInsert` where those types fit.
  ...
  it differs from the full table shape. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Define public request and response schemas separately from persistence types. An inferred insert
  type describes database inputs; it does not authorize a caller to set every available column. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep server runtime exports separate from shared input schemas and type-only imports. A UI
  component can consume a safe result type without importing the database connection. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

## Design the schema

Use database constraints to enforce the relationships and value rules that every writer depends on. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Give each entity a primary key. Choose identity integers, UUIDs, or another supported identifier
  ...
  replace a primary-key or unique constraint. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use foreign keys for relationships whose target belongs in the same database. Point them at a
  valid primary or unique key. Make a required reference non-null. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Enforce one-to-one cardinality with uniqueness on the referencing key. A relation declared as
  ...
  be absent even when its own parent reference is required. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- For many-to-many relationships, use a junction table with foreign keys and a composite primary key
  or unique constraint on the pair. A separate surrogate ID does not replace pair uniqueness. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Include tenant identity in constraints when a relationship needs to remain within one tenant. For
  ...
  membership as well as resource identity in the application. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Choose each delete and update action from the data's lifecycle. Use cascading deletion for
  dependent data that disappears with its parent. Preserve or restrict deletion of records
  with an independent retention requirement. Check the full cascade before enabling it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Give important constraints stable, descriptive names so errors and migrations identify the rule
  that failed.
  [PostgreSQL constraints](https://www.postgresql.org/docs/current/ddl-constraints.html). `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

### Preserve domain facts

- Store a fact once when several records refer to the same changing fact. Use related tables to
  avoid independently updating copies of names, memberships, or configuration. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Preserve intentional historical snapshots. A price agreed for an order or an address captured for
  a shipment has different meaning from the current product price or customer address. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Split values into fields when the application needs to validate, query, or update their parts.
  ...
  determines a city or that every country shares one address format. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use JSON for data whose flexible structure serves the feature. Keep values that need foreign keys,
  uniqueness, or frequent relational filtering in suitable columns or related tables. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Define how a polymorphic reference remains valid. A generic target type and target ID cannot use
  ...
  constrained references when those designs fit the domain. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- State how soft deletion affects queries, uniqueness, relationships, and retention. Keep active-row
  predicates consistent. Choose a partial unique index when uniqueness applies only to active rows. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

## Choose column types and defaults

Choose a PostgreSQL type from the value's range and meaning. Then choose a driver representation `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- `$type<T>()`, text enum inference, and `sql<T>` annotations describe TypeScript values. They do
  not validate database content or convert a driver's runtime result into that type. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Validate external JSON and values before writing them. Use database checks for required stored
  guarantees. Keep result decoding accurate for raw SQL, aggregates, custom types, and views. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use `NOT NULL` for required values. Pair it with a check constraint when the field also needs a
  range or other predicate. A check that evaluates to SQL null does not reject a null value. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Choose null behavior for unique constraints explicitly. Ordinary PostgreSQL uniqueness permits
  multiple null values. Use supported `NULLS NOT DISTINCT` semantics when nulls need to conflict. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep SQL null, an omitted update field, and an explicit replacement value distinct. Reject an
  empty patch when the operation needs at least one change. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- A PostgreSQL enum declared through `pgEnum` is a database object. Apply the TypeScript ban on
  language `enum` declarations to TypeScript source; choose database enum usage by schema needs. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

### Put defaults at the correct layer

Use database defaults or triggers when the behavior needs to apply to every writer, including SQL `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- `.default(...)`, `.defaultNow()`, and supported identity or UUID defaults produce database
  behavior when represented in the migration. Inspect the emitted SQL. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- `$defaultFn()` and `$onUpdate()` run through Drizzle ORM. They do not create database defaults or
  ...
  Drizzle before choosing them. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Define both insert and update behavior for an update timestamp. Verify an insert that omits it and
  an update made through each supported writer. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Share column helpers only when their types, defaults, and nullability have the same meaning. Keep
  table-specific constraints visible at the table declaration. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use a collision-resistant identifier generator and enforce uniqueness. Use cryptographically
  secure randomness for security tokens. Keep `Math.random()` out of token generation. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle column types and defaults](https://orm.drizzle.team/docs/column-types).

### Derive runtime validators

Use the installed Drizzle validation integration to derive structural checks when that avoids `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Choose `createInsertSchema`, `createUpdateSchema`, or `createSelectSchema` for the operation.
  Narrow generated input schemas to the fields the caller may set, then add domain validation. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Parse incoming values and pass the parsed result to the database operation. Constructing a
  validator without calling it does not validate input. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Match result validation to the selected columns. A full-table select schema cannot validate a
  projection that intentionally omits required columns. Handle an absent row before parsing it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Check field overrides carefully. Replacing a generated field schema can replace its nullability
  and optionality; callback refinements and replacements have different effects. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep coercion deliberate. Define accepted date, numeric, and boolean input formats rather than
  enabling broad coercion to make invalid requests pass. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle Zod integration](https://orm.drizzle.team/docs/zod).

### Define generated columns and custom types

Use a database-generated column when PostgreSQL computes the value from the row. Verify `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Keep generated fields out of writable request schemas. A generated expression has different
  behavior from an insert default or an ORM callback. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Review the emitted migration when adding or changing `.generatedAlwaysAs(...)`. Check expression
  restrictions, existing-row behavior, dependencies, and index changes before applying it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep SQL expressions deterministic within the database's generated-column requirements. Use
  another mechanism when the value depends on a subquery or external service. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Use a custom type only when built-in types and mappings do not represent the value correctly. Define `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Implement supported `toDriver`, `fromDriver`, or codec behavior for the actual conversion. Keep
  SQL type configuration under application control. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Verify round trips through ordinary selects, writes, and nested relational results. JSON-based
  result assembly can use a different conversion path from a direct column read. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Check precision, null values, invalid input, and serialization through the public API. Prevent
  double conversion when both a driver codec and a column mapping are active. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Add required extensions or database types through migrations before using the custom column. A
  TypeScript mapping alone does not install a PostgreSQL type. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle custom types](https://orm.drizzle.team/docs/custom-types).

## Define relations

Use Drizzle relations to describe how queries assemble related objects. Define database foreign keys `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Match the relation's source and target columns to their domain meaning. Give multiple links
  ...
  uses its own foreign-key column. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep nullable results accurate. A non-null reference does not guarantee a visible related row when
  ...
  query's full behavior guarantees a result. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use junction-table relationships for many-to-many reads. Keep the junction constraints even when a
  relation API hides the junction from the returned shape. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Treat predefined relation filters as query behavior. Keep authorization checks at the operation
  and database policy that enforce them for all callers. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- When splitting relation declarations, follow the installed release's composition rules. Check that
  ...
  configuration. Verify inference and representative nested queries after a split. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Select and limit related collections deliberately. A nested query that uses one SQL statement can
  still scan many rows, produce large JSON results, or repeat expensive work. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle relations](https://orm.drizzle.team/docs/relations).

## Write queries

Choose SQL-like or relational queries according to the required result. Keep predicates, `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Select the columns the operation needs. Return a safe projection to clients instead of spreading
  every table column into a response. Apply the same rule to nested relations and views. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Put resource and tenant restrictions in the query or enforced database policy. A user-provided row
  ID alone does not establish permission to read it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Bound page sizes and nested collection sizes. Validate page and cursor input before calculating
  limits. Use deterministic ordering with a unique tie-breaker. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use offset pagination for bounded results or deliberate page-number navigation. Use keyset
  ...
  null ordering, and cursor comparison consistent. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Add an optional filter by checking whether it is present. A numeric maximum of zero or a boolean
  value of false can be valid input. Do not drop those filters through truthiness checks. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Define whether text search treats `%` and `_` as wildcards. Escape them when the input represents
  a literal pattern. Parameterization protects SQL syntax but does not change wildcard behavior. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Handle join multiplicity and nullability. A left join can produce no child; joining several child
  ...
  with missing and repeated relationships. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Distinguish an absent row, an empty collection, an access denial, and a database failure. Map them
  to the operation's documented outcomes without revealing private resource existence. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Combine predicates explicitly with `and()` and `or()`. Do not assume repeated `.where()` calls
  accumulate conditions. A query helper must preserve tenant and resource restrictions. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use `.$dynamic()` when a helper needs to extend a query builder. It relaxes builder typing; it
  does not automatically merge clauses or make a mutable builder safe to share between requests. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep helper inputs and results typed with the installed dialect's query-builder types. Preserve
  changes to the selected shape when a helper adds joins or fields. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Create builders for the current operation. Avoid storing a mutable, request-filtered builder in a
  shared module. Inspect the completed SQL when several helpers contribute clauses. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle dynamic query building](https://orm.drizzle.team/docs/dynamic-query-building).

Use a subquery or common table expression (CTE) to name a meaningful intermediate result. Give SQL `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Use Drizzle operators and the `sql` template to bind data values. Build optional predicates from `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
typed expressions. Map user-selected sort fields to an allowlist of known columns and directions.

- Keep user strings, claims, and serialized JSON out of `sql.raw()`. Pass them as bound values. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- SQL identifiers are not ordinary value parameters. Choose tables, columns, role names, and SQL
  keywords from trusted application definitions; do not accept arbitrary identifier text. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use `sql.raw()` only for trusted SQL whose structure is controlled by the application. Keep the
  fragment small and explain why the query builder cannot express it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use runtime decoding or the driver's supported mapping when an expression returns a different
  ...
  string count into a number. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Define an explicit input schema for each create and update operation. Allow callers to set only
  ...
  server context. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Avoid accepting `Partial<SelectRow>` as a public update schema. It can expose IDs, ownership,
  counters, generated fields, and other server-controlled values. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Use atomic expressions or a version predicate for concurrent changes. Avoid reading a counter into
  JavaScript and writing it back when another request can change it in between. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Map expected constraint errors through their structured codes and constraint names. Keep raw SQL,
  parameters, and database diagnostics out of public errors. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Bound bulk insert and upsert sizes by parameter limits, payload size, memory, and transaction
  duration. Define whether all batches succeed together or whether partial progress is allowed. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Deduplicate an upsert batch by its conflict key before execution, using an explicit rule for
  competing inputs. Keep that key consistent with the database constraint. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Handle `onConflictDoNothing()` as a skipped insert. Its `returning` result may contain fewer rows
  than the input. Match results by stable keys instead of pairing arrays by position. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Distinguish the conflict-target predicate from the condition that allows an existing row to be
  updated. Use the installed API for each; preserve access restrictions on the update path. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Put changing values in supported `sql.placeholder(...)` parameters. Pass each request's values at
  execution time, including tenant and resource restrictions. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Give named statements stable names for stable SQL shapes. Avoid reusing a name for different
  statements or creating unbounded names from user input. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep a prepared query tied to the correct client and lifetime. A query prepared on a global client
  ...
  handle that owns its verified context. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Measure execution and planning costs. Preparation does not remove the need for suitable indexes,
  bounded results, or correct authorization. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle prepared queries and placeholders](https://orm.drizzle.team/docs/perf-queries).

## Use transactions

Group changes that need to succeed or fail together in a database transaction. Pass the transaction `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Use `tx` for every participating query. Calling a shared global client inside the callback can run
  work outside the transaction and outside its request identity. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Await all database work before the callback returns. Do not let background promises escape the
  transaction or assume parallel queries share a single connection safely. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep transactions short. Prepare input before opening one, and keep network calls, user
  interaction, and long-running provider work outside it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Choose isolation and locking from the concurrency invariant. Use a unique constraint for
  ...
  read-modify-write operation that spans rows. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Retry only failures whose outcome and driver behavior make retry safe. Retry the whole transaction
  ...
  callbacks free of irreversible external side effects. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use an idempotency key when a caller can repeat a write. Persist its outcome under a unique
  ...
  the transaction, then process it with duplicate handling. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Publish cache invalidation and success only after commit. A database rollback does not reverse an
  email, provider charge, or already published message. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use the selected driver's supported rollback and savepoint behavior. Do not catch a failed SQL
  statement and continue an aborted transaction as though it remained usable. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle transactions](https://orm.drizzle.team/docs/transactions).

## Control query caching

Enable Drizzle query caching only for operations with a defined freshness and invalidation policy. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Inspect the installed cache adapter's keys and supported query paths. Include every value that
  affects the result, including verified identity when results depend on it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- For RLS-backed reads, verify identity isolation explicitly. SQL and bound parameters can be
  ...
  path unless its keys and authorization preserve isolation. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Handle invalidation for writes outside the cached client, including raw SQL, transactions,
  ...
  queries and views rather than assuming every query participates. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Invalidate committed data and define an expiry as a fallback. Check races between a cache fill and
  a write. Keep transaction reads out of caches that cannot preserve transaction visibility. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Verify result serialization, cache failure behavior, and permission changes with two users. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Use read replicas for operations that tolerate replication delay. Keep writes and reads requiring `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- With `withReplicas()`, inspect which operations the installed adapter sends to replicas. Use
  `$primary` or the primary handle for reads that must observe a preceding write. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep authorization decisions that require current membership or revocation state on a suitable
  authoritative path. A stale replica can retain access data after it changes on the primary. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Execute every statement in a transaction through its transaction handle. Do not split a unit of
  work between the primary and an independently selected replica. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Verify schema rollout, roles, extensions, and connection limits for each endpoint. Define how
  replica failure affects the operation and whether fallback load is safe for the primary. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Test a write followed immediately by a read, delayed replication, and replica failure. Include
  cache behavior when a replica result can populate a cache. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Account for indexes created by primary keys and unique constraints. Avoid adding another index
  with the same columns and order without a measured reason. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- PostgreSQL does not automatically index the referencing side of every foreign key. Consider those
  indexes for joins, child lookups, and parent deletion checks. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- A junction primary key on `(user_id, group_id)` already supplies an index for that ordered pair.
  ...
  single-column indexes and another copy of the pair index. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Match composite index order to the workload. Consider tenant restrictions, equality predicates,
  range predicates, and pagination ordering together. Verify the plan with realistic selectivity. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use partial and expression indexes only when queries use compatible predicates and expressions.
  Include RLS predicates in performance review because they contribute to the executed query. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Measure read benefit against write cost, storage, and maintenance. A single SQL statement is not
  proof of efficient execution. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Inspect a plan with representative data. `EXPLAIN ANALYZE` executes the statement; use isolated
  data for writes and account for triggers or external effects before running it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Plan large index builds around locking and the migration runner. PostgreSQL
  ...
  recovery from a failed concurrent build before deploying it. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [PostgreSQL index creation](https://www.postgresql.org/docs/current/sql-createindex.html).

## Define views

Use a view for a reusable database projection or query. Select its columns explicitly, including `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Review access to sensitive columns before granting access to a view. A convenient `select *` can
  include credentials, internal flags, or new columns that were never intended for callers. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Treat a materialized view as stored results with a freshness policy. Define who refreshes it, when
  ...
  independently of the query that populated them. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use concurrent refresh only when PostgreSQL's requirements are met, including a populated view and
  a qualifying unique index. Coordinate refresh attempts for the same view. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Choose one schema authority and one runner that records applied migrations for each database. Keep `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
Use `drizzle-kit generate` to generate migration files, `migrate` to apply the selected Drizzle `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Configure the dialect, schema discovery, output directory, and target environment explicitly. Keep
  configuration secrets out of version control and command output. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep SQL, snapshots, and journals together in the format required by the installed Kit release. Do
  not mix artifacts from different migration formats or delete snapshots to force a clean diff. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- When Supabase CLI applies migrations, produce files in the format and naming convention it
  ...
  Supabase's. Use a deliberate generation or conversion step and verify replay through Supabase. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep one applied-history ledger authoritative. Do not alternate Drizzle `migrate` and
  `supabase db push` against the same change set as interchangeable runners. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Review the selected Supabase project and pending migration list before remote application. Keep
  remote migration execution in a deliberate task, separate from ordinary lint hooks. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Preserve Supabase-managed schemas, roles, and objects, including authentication infrastructure.
  ...
  object declarations or provider helpers for references, and inspect the resulting SQL. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Establish a baseline for an existing database by comparing its real schema with the intended
  ...
  creates or add `IF NOT EXISTS` broadly to hide an unexplained mismatch. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
1. Update the schema or authoritative SQL and generate the required artifacts with installed tools. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
5. Apply the migration only when migration execution is requested. Use the selected runner and
   report its result. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

Move required data directly to the new schema. Coordinate the schema and application change as one `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Keep applied migrations immutable. Correct a deployed schema with a new migration. Resolve
  ...
  metadata requirements. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Include policies, grants, functions, triggers, and custom SQL in versioned changes. A dashboard
  edit that is absent from history cannot be reproduced by a fresh environment. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep migrations out of module imports, request handlers, Server Component rendering, and ordinary
  application builds. Coordinate deployment migration execution so several instances do not race. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use custom SQL where the schema generator cannot express the required database operation. Keep
  that SQL in the same reviewed migration history and check future diffs for conflicting changes. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Separate disposable seed data from production reference-data migrations. Keep development seeds
  deterministic and free of real user data or production credentials. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

## Enforce row-level security

The row-level security contract (enable RLS on every exposed table, `USING` for visibility,
`WITH CHECK` for proposed rows, explicit grants, `SECURITY DEFINER` rules) is the database's rule
and applies unchanged here. Drizzle adds:

- When creating tables through Drizzle, inspect the generated migration for actual RLS enablement
  and policies. A table declaration does not enable RLS by itself. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Test with the role the application uses. Superusers and roles with `BYPASSRLS` bypass row
  ...
  not establish protection for application access. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep administrative access in separate, explicit operations. A policy naming a service role does
  not remove that role's bypass privileges. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Scope role generation carefully when using `pgRole` and Kit role management. Mark provider roles
  as existing through the installed API and inspect additions after provider or Kit upgrades. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- With RLS enabled and no applicable policy, ordinary row access is denied. Check that deployment
  creates the intended policy before exposing a new operation. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

See [Drizzle RLS declarations](https://orm.drizzle.team/docs/rls).
...
1. Verify the access token with the supported Supabase authentication API or a correctly configured
   ...
   not establish trust. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
2. Map verified identity to a small set of allowed application roles. Reject unexpected roles. Keep
   administrative roles out of this user-request path. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
3. Open a transaction using the intended restricted database connection. Set identity claims with
   ...
   Choose a role from fixed trusted SQL definitions, not interpolated token text. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
4. Execute all protected queries through that transaction's handle. Do not pass a global database
   client to a callback that expects the transaction's role and claims. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
5. Let commit or rollback end the transaction-local context. Verify that successful requests,
   failures, and pooled connection reuse cannot carry identity into the next request. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

Use Supabase's [verified claims API](https://supabase.com/docs/reference/javascript/auth-getclaims) `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
...
- Bind JSON claim values as parameters. Do not embed `JSON.stringify(token)` in `sql.raw()` or quote
  an interpolated raw fragment to construct a settings statement. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep role selection separate from value binding. SQL role identifiers require a controlled
  allowlist and supported identifier handling; they are not arbitrary string parameters. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Use transaction-local settings from the start. Do not depend on cleanup SQL to undo persistent
  ...
  rolled back. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Keep the authenticated user's connection path separate from administrative access. Avoid an API
  that casually returns both handles to every caller. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
- Test the bridge with malformed and expired tokens, unexpected roles, two different users,
  cross-tenant requests, forced query failures, and reuse of a small connection pool. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`

## Verify database behavior

Do not add a verification pipeline, fixtures, or mandatory evidence records when adopting Drizzle. `enforced-by: typescript/eslint drizzle/enforce-delete-with-where`
