...
```bash
quantize_args="--model ${model_name} --out ${checkpoint_dir}"
python -m src.quantization.vllm.quantize ${quantize_args}
```
...
- Ordinary stop and restart paths stop only PIDs recorded by the owning script
  family.
- A dedicated-host reset selected explicitly with `--reset` may use broad
  `pkill -f` engine-helper matches, terminate all NVIDIA compute PIDs, and reset
  the GPU. Those operations belong only in the reset process owner, never in a
  normal stop path.
- Reset is disabled by default. It attempts every cleanup stage, reports
  failures, and returns non-zero when any required stage fails.
...
- Keep pipeline state readable: model name, engine type, quantization mode,
  output directory, checkpoint path, Hugging Face repository, commit, Docker
  stack, and config path.
