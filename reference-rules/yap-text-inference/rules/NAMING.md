...
general naming principles plus Python, Bash, model artifacts, Hugging Face,
Docker, runtime, and test naming rules.
...
| `Dataset`    | A dataset object used for calibration, quantization, or evaluation. |
| `Example`    | One concrete example in calibration, testing, or docs.              |
| `Evaluation` | A result, metric, or workflow that measures runtime behavior.       |
| `Tokenizer`  | A Hugging Face tokenizer object or factory.                         |
| `Model`      | A model object, supported model name, or artifact.                  |
| `Engine`     | A runtime backend such as TRT or vLLM.                              |
| `Variant`    | A supported runtime or quantization variant.                        |
| `Client`     | External API, SDK, Hugging Face, filesystem, or platform boundary.  |
| `Loader`     | Reads data from a repository-owned source into in-memory values.    |
| `Parser`     | Converts raw input into structured data.                            |
| `Validator`  | Checks a value and returns or raises validation failure.            |
| `Formatter`  | Converts a value into display, log, model-card, or wire text.       |
...
- Shell scripts use lowercase `.sh` names. Existing numbered pipeline steps use
  `NN_description.sh`, such as `01_install.sh`.
...
- Source modules under `src/` are named for cohesive capabilities:
  configuration, engines, execution, handlers, Hugging Face publishing,
  quantization, runtime, state, telemetry, text, and tokens.
...
```text
src/engines/trt/setup.py
src/hf/push.py
src/quantization/vllm/artifacts/model.py
scripts/steps/03_start_server.sh
```
