...
These rules apply to all TypeScript in this project: API code, Supabase
scripts, Supabase Edge Functions, Supabase tests, shared lint tooling, and any
future TypeScript subproject.

## Contents

- [Core TypeScript Philosophy](#core-typescript-philosophy)
- [Scope](#scope)
...
- [Classes](#classes)
- [Types and Inference](#types-and-inference)
...
- [Rules Not Adopted](#rules-not-adopted)
...
- application and service TypeScript source
- TypeScript tests
- TypeScript scripts and migration generators
- Edge Function TypeScript
- shared TypeScript or TypeScript-adjacent tooling

Subproject-specific rules still apply. For API transport, Zod contracts, logging,
async, HTTP, and provider behavior, follow `rules/API.md`. For Supabase
migrations, Edge Function auth, generated database types, and deployment flows,
...
are not introduced, type declarations live in the required `types/` roots, and
...
TypeScript naming rules live in [`NAMING.md`](NAMING.md). Follow that file for
identifier casing, filename casing, module names, type names, constants,
boundary names, unused parameters, and TypeScript-specific source material
decisions.
...
- Keep generics minimal; follow [`NAMING.md`](NAMING.md) when a single `T` is not enough.
...
API-specific async, logging, provider, and HTTP error handling rules live in
`rules/API.md`.
...
Follow `rules/API.md` and `rules/SUPABASE.md` for subproject-specific testing
patterns.
...
The following external-guide rules are not adopted here:
...
- Do not introduce broad naming-lint policy changes as part of normal feature
  or docs work.
- Do not do opportunistic code refactors to make unrelated code match this guide.

These choices define the TypeScript standard. Enforcement should be updated when
it does not match the standard.
