...
These rules apply to all subprojects and all files in the monorepo.
...
case conventions, boundary names, SQL identifiers, and test names.
...
Error messages visible to clients (HTTP responses, database exceptions, API errors) must never leak internal system details. This applies to all subprojects.
...
- Keep messages consistent in tone and casing across all subprojects

See `rules/SUPABASE.md` and `rules/API.md` for subproject-specific error handling rules.
...
- It uses mocks only for external boundaries (APIs, databases, file systems, network). Mock the boundary, test the logic.
...
- **Access command.** Permissions, ownership rules, and role-specific behavior.
...
Bad: `// Removed the old polling logic.`
Good: `// Fetches updates via WebSocket.`
...
Exceptions: well-known ecosystem or platform configuration files may be
mentioned by name when genuinely useful.
...
| Language                  | What requires a doc comment                                                                             |
| ------------------------- | ------------------------------------------------------------------------------------------------------- |
| **SQL**                   | Every `CREATE TABLE`, `CREATE FUNCTION`, and `CREATE TRIGGER`. Section header, label, and purpose line. |
| **Swift**                 | `public` and `open` declarations only. Internal and private declarations do not require doc comments.   |
| **JavaScript/TypeScript** | Exported functions only. Non-exported functions do not require JSDoc.                                   |
...
This applies to all languages: JSDoc (`/** */`), Swift doc comments (`///`),
and SQL section headers (`-- Function: ... -- Purpose: ...`).
