...
steps, deployment pipelines, package-coordinator script bodies, Make recipes that
invoke Bash, Docker `RUN` blocks that rely on Bash behavior, and one-off shell
snippets committed to the repository.

Service-specific commands stay in the owning rule file. Bash syntax, error
handling, quoting, process execution, deployment-script safety, and shell review
standards live here.
...
- [Docker RUN Blocks](#docker-run-blocks)
...
- Do not hide production behavior in mise task bodies, Docker `RUN` lines, or CI
  YAML. Move non-trivial logic into a reviewed Bash script.
- When writing scripts, write Bash. Do not create Python, Ruby, Perl,
  JavaScript, TypeScript, Swift, or other scripting-language files for script
  work.
...
#!/usr/bin/env bash
#
# Deploy API static assets to the configured host.
...
- Separate validation, planning, confirmation, execution, readiness checks, and
  cleanup.
- Fail before doing work when required inputs, commands, files, hosts, or
  secrets are missing.
- Make the target environment explicit. Do not infer production from a branch
  name without a clear confirmation or CI contract.
- Make destructive production actions require explicit confirmation unless CI
  owns the guard.
- Keep deployment state readable: environment, host, service, artifact, commit,
  image tag, migration range, and config path.
...
- Keep rollback or teardown commands explicit. Do not invent automatic rollback
  unless the platform supports it and it has been verified.
...
- Validate hostnames, usernames, service names, container names, and systemd unit
  names before using them in remote commands.
...
```bash
./quality/security/codeql/run.sh api
./quality/packages/licenses/check.sh
```
...
- If a `RUN` pipeline matters, either use Bash with `pipefail` for that block or
  avoid the pipeline.
- Pin downloaded tool versions and verify checksums when practical.
- Clean package-coordinator caches in the same layer.
...
- Account for macOS/BSD and GNU differences in `sed`, `date`, `readlink`,
  `mktemp`, `stat`, `xargs`, `grep`, and `encodedBytes`.
...
- Do not use Linux-only utilities in iOS/macOS scripts without checks.
...
Run the owning project's checks from the location expected by that project.
...
- Do not move Bash logic into Python, Ruby, Perl, JavaScript, TypeScript, Swift,
  or other scripting languages to make it easier to test.
...
- Runtime trial runs are allowed only when they are part of the requested
  workflow or needed to verify a real deploy/local command, not as a new test
  suite.
...
2. Identify the caller contract: local dev, CI, remote host, Docker, or package
   script.
