...
These rules apply to all files in the Yap landing repo.
...
Read the relevant code before touching it. Understand the contracts, data flow, and ownership boundaries. Then think through your approach:
...
Before adding a new module, directory, or helper, identify the correct owner for the behavior. Do not create parallel implementations for a concept that has an owner. If a new file is needed, be ready to explain why the owning module should grow that way.
...
- Before adding a new helper, find the owner of the behavior and put the logic there.
- When touching duplicated logic in the same area, collapse it into the owner instead of adding another layer.

## Naming

All naming rules live in [`NAMING.md`](NAMING.md). Follow that file for identifiers, files, directories, functions, booleans, role suffixes, language case conventions, static-site boundaries, asset paths, route names, and test names.

The quality tooling under `quality/repository/naming` is also authoritative for naming and banned-term policy.

## Abstractions

Abstractions are useful only when they remove real complexity. They are harmful when they hide ownership, combine unrelated behavior, or predict reuse before the code proves it.

### Prefer Duplication Over the Wrong Abstraction

- Duplication is cheaper than the wrong abstraction.
- Prefer duplication until there are at least two real examples that prove the same concept exists.
...
- If a shared abstraction starts gaining flags, modes, optional branches, or caller-specific conditionals, treat that as evidence the abstraction is wrong.
- "Boxing" is forbidden: do not stuff loosely related behavior into one function/class/module with parameters deciding which behavior runs.

### Inline the Wrong Abstraction

- When an abstraction is wrong, inline it back into each caller, delete the branches each caller does not need, then extract only the common behavior that remains.

### Make the Change Easy

- Preparatory refactoring is allowed when it makes the requested change easier: first preserve behavior, then make the behavior change.
- Keep refactoring and behavior changes separate when practical.

### Keep Granularity Continuous

- Higher-level helpers must be replaceable by a small number of lower-level operations. Do not create abstraction granularity gaps.

### Use Inversion of Control Deliberately

- Use inversion of command when it prevents option explosion across multiple real use cases.
- Do not add inversion of command for a single use case if it makes the call site harder without reducing complexity.

### Prefer Data Flow and Data Structures

- Prefer plain functions and explicit data flow before classes, interfaces, factories, strategies, inheritance, or framework patterns.
- Prefer data structures and their relationships over code-pattern taxonomies.
- Push state and I/O outward where practical; keep core logic pure or close to pure when that reduces moving parts.
...
Error messages visible to clients, generated pages, Cloudflare middleware responses, local server responses, and build tooling must never leak internal system details.

**Never include in error messages:**

- Internal route, asset, file, or function names
- Internal identifiers, tokens, or private configuration values
- Stack traces or file paths
- Implementation details such as generated path internals, build pipeline state, or quality tool paths
...
- Keep messages consistent in tone and casing across the repo
...
- It uses mocks only for external boundaries such as file systems, network calls, analytics providers, and platform APIs. Mock the boundary, test the logic.
...
- **Request/response behavior.** Contracts, error handling, rate limiting, caching, authentication flows, and edge cases.
...
- That a storage field has a specific default value by reading it back and comparing.
...
Bad: `// Removed the old polling logic.` Good: `// Fetches updates via WebSocket.`
...
Exceptions: well-known configuration files such as `package.json`, `mise.toml`, `.commitlintrc.json`, and `.markdownlint-cli2.jsonc` may be mentioned by name when genuinely useful.

### Punctuation

Do not use em dashes or double hyphens. Use a space, comma, or colon instead.

Bad: `The server handles requests - including retries - before responding.` Good: `The server handles requests, including retries, before responding.`
...
- **File or variable locations.** Do not say "defined in another file" or "see the value in config.Y" unless the reference is essential for understanding. Code is searchable; stale path references are not.
...
Doc comments are required at different scopes depending on the language. Linters enforce the mandatory level; human review covers the rest.

#### Linter-Enforced (Mandatory)

| Language       | What requires documentation                                                      |
| -------------- | -------------------------------------------------------------------------------- |
| **JavaScript** | Exported functions only. Non-exported functions do not require JSDoc.            |
| **Bash**       | Executable files require a file header, and functions require one-line comments. |
...
This applies to all repo-owned JavaScript JSDoc (`/** */`) and Bash comments.

## Protected Files

The following root-level files must never be modified unless the user explicitly asks for it:

- `.license-checker.json`: controls the allowed license list for all subprojects.

Do not add, remove, or change entries in these files on your own initiative, even if it would unblock a dependency or fix a warning.
