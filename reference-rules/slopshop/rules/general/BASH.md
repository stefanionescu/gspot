...
| Support developer machines and CI    | [Runtime compatibility](#runtime-compatibility), [portability](#portability-rules), [CI scripts](#ci-scripts)                                                                         |
...
- Keep structured-data parsing and source-code analysis in their existing JavaScript modules.
...
cd "${REPO_ROOT}"
exec mise run repo:hook:pre-commit -- "$@"
```
...
If a workflow needs nested maps, large arrays, non-trivial validation, or domain
rules, implement that behavior in the owning application or JavaScript quality module.
...
Use the Bash version configured for the project's development and deployment environments.
Use that version's supported features directly. Do not target Bash 3.2 or add legacy syntax paths,
version-selection wrappers, or compatibility implementations.

If a required Bash feature is unavailable, correct the environment requirement instead of keeping
...
For a script with functions, use this order. Omit sections that the script does not
need. A hook or mise task that only invokes one command can call it directly.
...
6. Functions.
7. `main`.
8. `main "$@"` as the last non-comment line when the executable script defines `main`.
...
- Do not toggle options globally around a small operation without restoring the
  prior state.
- Do not change `IFS` globally just to enable a so-called strict mode. Set `IFS` locally
...
When several callers need the same logging behavior, a shared function can format it:
...
Document functions immediately above their declaration. Preserve the configured
exception for `main`. Use the required `# function_name: description` form:

```bash
# normalize_env_name: Converts an environment alias to the configured name.
normalize_env_name() {
  ...
}
```
...
# upload_asset: Uploads one asset to remote storage.
upload_asset() {
  ...
}
```
...
Use `main` for the entry point of an executable script with functions.
Simple hook and mise entry files can invoke their command directly; do not introduce
trivial functions just to wrap it. Follow the configured shell file and function limits.
Document other functions immediately above their declaration with
...
- Use `readarray` or a `while read` loop for line-based input.
- Do not simulate nested maps or records with shell arrays. Use the JavaScript module
  responsible for that structured data.

For line-based records, `readarray` reads one value per line. This
example assumes paths cannot contain newlines. For arbitrary filenames, use the
NUL-delimited example below. Also check the producer status when its failure matters:

```bash
readarray -t files < <(find . -type f -name '*.sql' -print)
```

Use a loop when each line needs processing:
...
- Prefer function-local `IFS` or a subshell when a temporary separator is
  needed.
- Do not parse general CSV with `IFS=, read ...`; use an owned JavaScript module
  with a real CSV parser.
...
```bash
content_with_sentinel="$(some_command || exit "$?"; printf x)" || return "$?"
...
- When shell features are required, use a static Bash snippet and pass
...
```bash
declare -a cmd
cmd=(docker compose --project-name "${project_name}" ps --all)
"${cmd[@]}"
```
...
- Prefer service-manager commands, PID files owned by the script family,
  `pgrep`/`pkill` with exact matching, or Docker/systemd APIs.
...
- Keep per-job output in separate files when concurrent jobs can interleave
  logs.
- Limit how many jobs run at once. When targeting many hosts or files, use an explicit
...
```bash
./linting/security/codeql/run.sh api
./linting/licenses/check.sh
```
...
- use unsanitized values as variable names, associative array keys in arithmetic
  contexts, systemd unit names, Docker compose service names, SQL filenames, or
  remote shell fragments;
...
2. Identify the caller and environment: local development, CI, a remote host, a container,
   or a package script.
...
7. Do not convert a large script in one pass unless the task is explicitly a
   script cleanup.
8. Use the configured Bash runtime. Change shebangs only when the requested implementation needs it.
9. Run lint or formatting only when explicitly requested, limited to the affected scripts.

When a script is too complex:

- keep the Bash wrapper thin;
- move parsing into the owning JavaScript quality module and business logic into application code;
