...
Use these rules when writing or reviewing application code, services, tests, scripts, migration
generators, and shared tooling written in TypeScript.
...
- [Write tests and mocks](#write-tests-and-mocks)
...
Use Zod, project-specific validation functions, or database constraints to check incoming values.
After validation, pass the resulting typed values to the code that uses them.
...
| Shared JavaScript lint tooling      | Follow the tooling's JavaScript ESLint configuration                                                                                |

Keep the exact ESLint rule configuration with the lint tooling. Apply the language standards in this
file consistently: explicit type-only imports and exports, `type` aliases for object shapes, runtime
validation of external input, safe narrowing, and the naming conventions below. Keep TypeScript
enums out of application code.
...
A barrel is an index module that re-exports symbols from other modules. Keep each barrel focused on
one public API. Avoid root barrels that collect unrelated features and pull extra dependencies into
callers. Import the file that defines a symbol when that is the surrounding code's pattern.

Follow the [runtime import policy](#configure-the-compiler-and-runtime) when choosing extensions.

## Place type declarations

Put a reusable type beside the shared behavior it describes or in the subproject's established type
directory. Keep generated types, test-only types, and shared handwritten types in their respective
modules.
...
Validate values at the point where external data enters the application. Use Zod or the project's
runtime validation functions for:

- HTTP bodies, query strings, route parameters, and headers;
- environment variables;
- database rows and RPC results that have not already been validated;
- provider responses;
- file input and generated data sources;
- webhooks and scheduled-job payloads.
...
Create, update, or run tests only when explicitly requested. For requested tests, cover observable
behavior and the outcomes callers depend on.

- Keep shared test types with the tests that use them, separate from production types.
- Use `vi.mocked()` or typed local helpers when accessing Vitest mocks.
- Mock external dependencies as needed. Keep internal implementation details out of mock contracts.
- Avoid `any` in tests. Use `unknown`, typed fixtures, or narrowly typed mock helpers.
- Use non-null assertions in tests only when the setup clearly establishes that the value exists.

## Maintain generated code

Regenerate database types and other generated TypeScript through the tool that owns the output. Keep
schema changes and regenerated types together.

- Leave generated database type files to their generator.
