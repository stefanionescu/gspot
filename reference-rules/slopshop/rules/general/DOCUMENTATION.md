...
| Document an interface or release             | [Commands, APIs, and configuration](#document-commands-apis-and-configuration), [release changes](#document-releases-and-lifecycle-changes)                                                                        |
...
directory names seem stable, or an older document already contains one.
...
readers need that history to understand a decision or complete a migration.
...
Check important examples using the normal project workflow when practical. Follow
[GENERAL.md](GENERAL.md#verification) for the checks required by the current change.
...
### Preserve links when headings change
...
- Use the new heading directly. Do not retain old anchors, redirects, or compatibility aliases.
...
Contributor documentation must include:
...
A known-issue note must include:
...
- Update the maintained documentation.
...
Do not add datestamps to ordinary guides and references. Version control already
...
Update only existing documentation that the requested change makes inaccurate. This may include a
comment, guide, example, or diagram. Do not create new documentation layers, generated references,
troubleshooting pages, or diagrams just to complete a checklist.
...
Document the current behavior. Do not maintain parallel documentation for old versions.
...
sentences. For a guide, follow the steps from the stated starting point. For a
reference, try finding and using a value without reading the whole page.
...
For substantive or high-risk documentation, ask someone from the intended audience to try the
document without coaching when a representative reader is available. Note where they hesitate,
misunderstand a term, or miss a prerequisite.
Revise the text and check those points again. If reader feedback is unavailable,
record that limit in the review instead of claiming that tools proved usability.
