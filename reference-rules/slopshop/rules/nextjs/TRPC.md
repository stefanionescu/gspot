...
Use these rules for procedure ownership, request context, authorization, transports, and tRPC integrations.
...
- [tRPC and TanStack Query](#trpc-and-tanstack-query)
- [Organize feature server code](#organize-feature-server-code)
- [Request context and protected procedures](#request-context-and-protected-procedures)
- [Configure the TanStack-native client](#configure-the-tanstack-native-client)
- [Compose procedures and validators](#compose-procedures-and-validators)
- [Call procedures on the server](#call-procedures-on-the-server)
- [Configure HTTP transport and batching](#configure-http-transport-and-batching)
- [Handle uploads and non-JSON input](#handle-uploads-and-non-json-input)
- [Control cancellation and retries](#control-cancellation-and-retries)
- [Manage tRPC subscriptions](#manage-trpc-subscriptions)
- [Use tRPC-backed Server Actions](#use-trpc-backed-server-actions)
...
- Keep context creation read-only when used during RSC rendering. Account creation, anonymous
  ...
  that can perform them. Do not silently sign in while reading route data.
...
- Make the data operation's access contract explicit. A query is either responsible for
  ...
  directly.
- Keep read and write paths distinguishable. A function named as a query should not initialize a
...
- Compose object input schemas only when their fields have a clear owner. Chained `.input()` parsers
  ...
  with a later transformation that changes which tenant the resolver receives.
...
- Define `.output()` where a runtime response contract needs enforcement, such as a provider result
  ...
  server and a safe client response.
