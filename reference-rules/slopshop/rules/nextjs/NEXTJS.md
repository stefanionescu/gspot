...
Use these rules for routing, Server and Client Components, rendering, data access, caching, and deployment in Next.js.
...
| Your change                       | Read                                                             |
| --------------------------------- | ---------------------------------------------------------------- |
| Routing, rendering, or components | [Application structure and rendering](#nextjs-application-rules) |
| Server reads, writes, or caching  | [Data access and caching](#data-access-writes-and-caching)       |
| Deployment or runtime             | [Deployment and runtime](#deployment-and-runtime)                |
...
### Verify streaming through the deployment
...
See
[streaming infrastructure and verification](https://nextjs.org/docs/app/guides/streaming#what-can-affect-streaming).

### Verify the deployment runtime

Verify the application on its production runtime and deployment adapter. Check how it is built and
which Node libraries, filesystem operations, and cache storage the environment supports. Confirm
request and stream size limits, and how long responses can remain open.

A production build must not require a developer's personal session or untracked secret files.
Provide documented nonproduction credentials or supported service fixtures for verification.
Environment configuration belongs to the runtime owner; never bypass validation with an empty string
simply to get a build to finish.

When deploying to Cloudflare, follow the supported adapter for the selected platform and Next
version. Workers uses the OpenNext integration; older Pages/next-on-pages configuration must not be
copied as a current Workers setup. Verify the adapter build and local worker preview, bindings,
compatibility flags, and deployed streaming behavior. `next dev` alone does not exercise the worker
runtime. See
[Cloudflare's Next.js guidance](https://developers.cloudflare.com/workers/framework-guides/web-apps/nextjs/).

Use instrumentation and safe logs to observe failures, latency, and resource usage. Keep
instrumentation modules in the environment they support. Browser telemetry must not import
server-only SDK initialization, and server telemetry must not place private data in
client-accessible payloads.
