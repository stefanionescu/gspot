...
Use these rules for server-state ownership, query hydration, caching, mutations, pagination, and cancellation.
...
- [Define query keys and fetch contracts](#define-query-keys-and-fetch-contracts)
- [Stream pending queries](#stream-pending-queries)
- [Recover queries through error boundaries](#recover-queries-through-error-boundaries)
- [Set query freshness and retention](#set-query-freshness-and-retention)
- [Schedule independent queries](#schedule-independent-queries)
- [Handle offline query work](#handle-offline-query-work)
- [Use initial and placeholder data](#use-initial-and-placeholder-data)
- [Select query data for each consumer](#select-query-data-for-each-consumer)
- [Update query caches after mutations](#update-query-caches-after-mutations)
- [Keep infinite query pages consistent](#keep-infinite-query-pages-consistent)
- [Persist query state deliberately](#persist-query-state-deliberately)
- [Reconcile paginated and streamed state](#reconcile-paginated-and-streamed-state)
...
Use framework server reads for data owned entirely by Server Components. Introduce a client query
...
- Use a request-scoped server QueryClient and a stable browser QueryClient. Never share a server
  ...
  Reset or partition browser caches when identity or tenant changes.
...
- Prefetch useful initial queries on the server and hydrate through the selected integration. Server
  ...
  supported dehydration behavior, not just passing a promise into arbitrary JSON.
...
- Set freshness by resource. Global infinite `staleTime` plus disabled refetch triggers is allowed
  ...
  distinguish initial load, next page, background fetch, empty result, and failure.
...
- Cursor keys include sort/filter/resource/locale where relevant. The next cursor comes from the
  ...
  appropriate. Do not maintain a competing cursor in a global store without a defined reason.
...
- Do not copy query data to a client store merely to access it in another component. For streaming
  ...
  reconciles, and how reset works.
...
- Trace the actual route from prefetch through dehydration to the consuming component. Defining a
  ...
  with the same request-local QueryClient.
- Check direct load and subsequent navigation separately. Avoid a stale-time workaround that hides
  an incorrect key, scope, or hydration boundary.

Choose retry and freshness settings for each resource. Retry transient read failures within a limit;
...
Give different response shapes distinct keys. A list preview and an editable detail record are not
...
Keep each key beside the fetch function and shared options that define its result. Use
...
Check unsuccessful HTTP responses before parsing them as successful data. `fetch` does not reject
...
Place HydrationBoundary around the components that consume its prefetched queries. Multiple server
...
- Await a read when its result determines the response, access decision, or content that the current
  ...
  only when pending-query hydration is configured.
- For supported TanStack releases, include pending queries alongside the default dehydration
  ...
  set further when some cached values are private to the server.
- Use `useSuspenseQuery` beneath Suspense when the consumer should wait for the server-created
  ...
  content merely because its promise was dehydrated.
- Keep a settled prefetch's fallback behavior explicit. An ordinary query can fetch in the browser
  ...
  with a missing or failed prefetch as well as the successful path.
- Pair `dehydrate.serializeData` with `hydrate.deserializeData` for supported non-JSON values. Keep
  ...
  serialize that result again at dehydration.
...
- Preserve Next.js control-flow errors during server rendering. Configure `shouldRedactErrors`
  ...
  serialized JSON and client-facing diagnostics.
- Check the installed package's exports before using guide-specific APIs such as
  ...
  required data loaded or that the route can safely continue.

Prefer explicit prefetching for routes where nested queries would otherwise wait on one another. If
...
consumer when required inputs exist. For an update that should retain revealed content, use a
...
Choose freshness from how long the user can safely see an old value. Choose retention from how much
...
Do not classify revocable permissions or changing account state as static reference data. Browser
...
Use generated tRPC filters or shared query-key factories to identify affected records, lists, and
...
Use an API request or tRPC query for browser reads. Keep Server Actions out of `queryFn`; their
...
Use `initialData` for a complete valid value that can populate the query cache. Preserve its actual
...
Use `placeholderData` for temporary display while the real query loads. It is observer display data,
...
Use `select` when a component needs a subset or derived value from a cached result. This narrows the
...
Use a stable selector reference when a costly transformation would otherwise rerun on every render.
...
Read the result properties the component needs. Object rest destructuring reads the remaining
...
Use mutation variables to identify the record being updated, rather than the current selection when
...
Place required cache updates in the shared mutation options or another operation owner that handles
...
Return or await required invalidation promises so the mutation's pending state covers the intended
...
Treat one infinite query as one cache entry containing aligned `pages` and `pageParams` arrays.
...
- Supply `initialPageParam` when using TanStack's direct infinite-query API. For tRPC factories,
  configure the installed integration's initial-cursor option and let it form the query options.
- Derive continuation from the server contract. Return `null` or `undefined` from page-parameter
  callbacks when that direction is exhausted. A valid cursor of `0` is not an exhausted page.
- Guard automatic loading with availability and `!isFetching`, not only `!isFetchingNextPage`.
  Background refetches share the same cache entry and can conflict with a next-page request.
- Treat `cancelRefetch` as coordination of repeated fetch calls. With `false`, a call during an
  ...
  can replace in-flight work. Do not use either setting as a parallel-page-fetch switch.
- Use `maxPages` when long sessions could retain excessive history or make refreshes expensive.
  ...
  Bound both the retained data and the work needed to refresh it.
- Configure previous-page fetching when the user needs to return to pages evicted from the start.
  ...
  unmounting DOM rows are separate operations.
- When removing a cache entry, expect pagination to restart from its initial page. Do not keep a
  global cursor pointing beyond pages that the query no longer holds.

Check background refresh during a scroll-triggered load, page eviction followed by reverse loading,
...
Add persistent query storage only for a defined offline or restoration need. Keep storage
...
Select which data may survive a reload. Partition or clear stored private data and paused mutations
...
Let an infinite query own fetched pages when the interaction is ordinary cursor pagination. Derive
...
- Use the complete cursor returned by the server, including tie-breaker IDs. Do not reconstruct it
  ...
  pages, and reset the page sequence when that scope changes.
- Derive the next page from the response contract. An empty final page still updates exhaustion
  ...
  record identity when concurrent updates can overlap page boundaries.
- Derive ordinary status from the query instead of copying success, loading, and error flags into
  ...
  captured operation scope; stale closures can request pages for the wrong resource.
- When streamed content needs a separate store, define the transition between server snapshots,
  ...
  successful result. A permanent `dataProcessed` flag can leave later snapshots unapplied.
- Capture the resource ID when a save starts. Keep that ID with the operation through its success or
  ...
  resource rather than whichever resource is selected when the promise resolves.
- Reconcile the mutation's returned canonical IDs, timestamps, positions, and versions. Replace or
  ...
  successful database write does not update local state merely because `mutateAsync` resolved.
- Preserve required invalidation and reconciliation when adding per-call callbacks. Await the
  ...
  repeating the mutation could duplicate it.
...
Check the installed hook's cancellation limits. TanStack documents cancellation as unsupported for
