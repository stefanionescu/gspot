...
Use these rules when defining schemas, querying data, managing migrations, or connecting an
application through Drizzle ORM. The database rules below cover PostgreSQL. Apply the Supabase
sections when Supabase hosts the database. Check the selected dialect before using these rules with
another database.
...
Refactor affected schemas, operations, and callers together when their contracts change.

## Contents

| Your task                                  | Read these rules                                                                  |
| ------------------------------------------ | --------------------------------------------------------------------------------- |
| Select packages or upgrade Drizzle         | [Check versions and APIs](#check-versions-and-apis)                               |
| Connect an application or migration runner | [Configure connections](#configure-connections)                                   |
| Place schemas and database operations      | [Organize database code](#organize-database-code)                                 |
| Define columns and relationships           | [Design the schema](#design-the-schema)                                           |
| Map database values to TypeScript          | [Choose column types and defaults](#choose-column-types-and-defaults)             |
| Fetch nested data                          | [Define relations](#define-relations)                                             |
| Read or change rows                        | [Write queries](#write-queries)                                                   |
| Coordinate several writes                  | [Use transactions](#use-transactions)                                             |
| Improve database performance               | [Design indexes](#design-indexes)                                                 |
| Expose a view or stored result             | [Define views](#define-views)                                                     |
| Change a deployed schema                   | [Manage migrations](#manage-migrations)                                           |
| Restrict rows by user or tenant            | [Enforce row-level security](#enforce-row-level-security)                         |
| Pass Supabase identity to SQL              | [Use verified identity in a transaction](#use-verified-identity-in-a-transaction) |
| Verify and deliver a change                | [Verify database behavior](#verify-database-behavior)                             |
| Validate requests or query results         | [Derive runtime validators](#derive-runtime-validators)                           |
| Add database-computed values or mappings   | [Generated columns and custom types](#define-generated-columns-and-custom-types)  |
| Extend queries through helpers             | [Compose queries safely](#compose-queries-safely)                                 |
| Reuse a fixed query shape                  | [Reuse prepared queries](#reuse-prepared-queries)                                 |
| Cache database reads                       | [Control query caching](#control-query-caching)                                   |
| Route reads to replicas                    | [Read from replicas](#read-from-replicas)                                         |
