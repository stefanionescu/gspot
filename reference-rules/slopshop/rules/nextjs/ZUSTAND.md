...
Use these rules for shared client stores, selectors, persistence, hydration, and request isolation.
...
- [Manage shared state with Zustand](#manage-shared-state-with-zustand)
- [Scope and initialize the store](#scope-and-initialize-the-store)
- [Update state without losing fields](#update-state-without-losing-fields)
- [Compose actions and slices](#compose-actions-and-slices)
- [Keep selector results stable](#keep-selector-results-stable)
- [Hydrate persisted store state](#hydrate-persisted-store-state)
- [Synchronize store state with the URL](#synchronize-store-state-with-the-url)
...
- Select the specific values and actions a component reads. Do not subscribe a large layout or every
  list row to the entire store with an unselected store hook.
- Prefer a single primitive/entity selector when possible. When a selector returns an object or
  array of several values, use the installed Zustand version's supported equality mechanism, such as
  `useShallow`, to avoid updates caused only by a newly allocated wrapper.
- Shallow comparison is not deep comparison. Keep nested object identity stable when it did not
  ...
  it did. Mutating a Map in place can prevent subscribers from seeing a real update.
- Subscribe a virtual row by stable entity ID when using normalized records. Subscribe the list
  ...
  should not rerender all rows and the shell.
...
- Keep related state transitions with their feature owner, normally as actions beside the state.
  ...
  store instance; they do not bypass request or provider scope through a hidden global store.
- Keep async request identity and cancellation explicit. A delayed store action must not update the
  next resource after the reader switches away.
- Reset private state on sign-out and identity/tenant changes. Persist only the fields that actually
  ...
  transient request flags, or live handles.
...
- Do not store virtualizer handles or control state on global DOM nodes. Pass the controller through
  its local owner, props, or an appropriately scoped context.
...
Pass a safe initial snapshot from server data into the provider. Use identical initial values for
...
Place the provider where the state should live. A shared layout can preserve state across
...
Use functional `set` updates when the result depends on current state. Zustand merges the returned
...
Use `replace: true` only for a complete replacement that preserves the store's required shape,
...
Give actions domain meaning: adding a record, switching a resource, or applying a confirmed save.
...
Use slices to organize one cohesive store. Give each field and action one owner so spreading slices
...
Use the equality API supported by the installed version. Zustand v5's standard `create` does not
...
Use `persist` with a unique storage name and a `partialize` allowlist of durable fields. Keep
...
Persist the current store shape. Reset disposable cached state when its shape changes instead of
...
Keep browser restoration separate from the initial server-compatible snapshot. Use `skipHydration`
...
Let the URL own shareable navigation values, such as filters and sorting. Persist only a selected,
...
Define precedence between URL values, stored preferences, and defaults. When the URL owns a value,
...
for transient edits and a new history entry when back navigation should restore the previous state.
