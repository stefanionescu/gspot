...
Use these rules for authorization, secrets, untrusted content, endpoint controls, and stream resource ownership.
...
- [Next.js security and runtime boundaries](#nextjs-security-and-runtime-boundaries)
- [Keep configuration out of browser bundles](#keep-configuration-out-of-browser-bundles)
- [Check authorization where data is accessed](#check-authorization-where-data-is-accessed)
- [Protect inference and retrieval endpoints](#protect-inference-and-retrieval-endpoints)
- [Render Markdown and rich editors safely](#render-markdown-and-rich-editors-safely)
- [Scope service workers and private media](#scope-service-workers-and-private-media)
- [Separate telemetry and provider results](#separate-telemetry-and-provider-results)
- [Endpoint and streaming lifecycle](#endpoint-and-streaming-lifecycle)
...
- Derive resource ownership and allowed model/provider choices on the server. Bound input bytes,
  context count, per-message size, output tokens, request duration, and concurrent work. Browser
  token estimates and disabled buttons are feedback, not quota enforcement.
...
| -------------------------- | ------------------------------------------------------------------------- |
| Server security boundaries | [Next.js Data Security](https://nextjs.org/docs/app/guides/data-security) |
