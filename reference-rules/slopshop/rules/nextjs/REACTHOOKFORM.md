...
Use these rules when building or reviewing forms with React Hook Form. Keep library-specific form
state, field registration, subscriptions, and submission guidance in this file.
...
| Your task                                         | Read these rules                                                                            |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Choose APIs or update dependencies                | [Check versions and integrations](#check-versions-and-integrations)                         |
| Set up a Next.js form                             | [Own the form instance](#own-the-form-instance)                                             |
| Load, refresh, or reset an editor                 | [Initialize and reconcile values](#initialize-and-reconcile-values)                         |
| Connect native or custom controls                 | [Register inputs and controlled components](#register-inputs-and-controlled-components)     |
| Validate with Zod                                 | [Define validation and parsed types](#define-validation-and-parsed-types)                   |
| Reduce subscriptions and renders                  | [Subscribe to the state a component needs](#subscribe-to-the-state-a-component-needs)       |
| Add, remove, or reorder rows                      | [Manage field arrays](#manage-field-arrays)                                                 |
| Build conditional, wizard, or virtualized forms   | [Preserve values across unmounts](#preserve-values-across-unmounts)                         |
| Save through an API or mutation                   | [Submit and recover](#submit-and-recover)                                                   |
| Use Server Actions                                | [Integrate Server Actions and useActionState](#integrate-server-actions-and-useactionstate) |
| Show errors and focus a field                     | [Present accessible errors](#present-accessible-errors)                                     |
| Share a form through context or explicit controls | [Compose form components](#compose-form-components)                                         |
| Use rich values in TypeScript                     | [Keep field paths precise](#keep-field-paths-precise)                                       |
| Verify a change                                   | [Test form behavior](#test-form-behavior)                                                   |
...
- Treat v7 and v8 beta documentation as separate contracts. Do not copy a beta signature into a v7
  application or infer availability from a documentation menu.
- Check version support for newer options. The supplied v7 documentation introduces
  ...
  `fields[].disabled` in 7.80. Verify the installed behavior before depending on them.
- Check `exact` subscription semantics during upgrades. The v7.81 documentation includes updates
  made to an ancestor path while excluding updates made to a nested child path when `exact` is true.
- `keyName` is a v7 compatibility option scheduled for removal in the next major version. Keep
  persisted identifiers independent of the generated field-array key.
- Verify newer helpers such as `setValues`, `resetDefaultValues`, `useLens`, and `OpaqueTypes`
  against installed exports and types before adopting them. Use them for a concrete need.
...
- Keep mutable form controls scoped to their mounted form. Do not create request-specific
  `createFormControl` instances as module singletons on a Next.js server.
- Give server rendering and the first client render the same defaults. Do not derive initial field
  values from browser storage, a random value, or a browser-only clock during rendering.
- Keep the query cache responsible for server snapshots and React Hook Form responsible for the
  editable draft. Do not mirror every keystroke into Zustand, query data, and local React state.
...
- Do not recreate the form to clear an ordinary validation error. Use the relevant error or reset
  API and preserve the user's work.

## Initialize and reconcile values

Provide complete `defaultValues` for the editable input model. They establish the baseline used by
...
- Use field-appropriate empty values, such as an empty string, empty array, or an explicitly allowed
  ...
  defaults unless the chosen control and equality behavior support them.
- Use async `defaultValues` for initial loading when that matches the data owner. Surface
  ...
  snapshot independently through both the form and TanStack Query.
- Use `values` only when external changes should update the form reactively. It can overwrite
  current edits and defaults. Choose `resetOptions` deliberately before connecting it to query
  results that can refetch in the background.
- Use `reset(nextValues)` when accepting a new editing baseline, including a confirmed save or a
  deliberate record change. Do not reset before form subscriptions have initialized.
- Use `keepDirtyValues` when a refresh should update untouched fields while preserving edits, and
  subscribe to `dirtyFields`. `keepDirty` preserves dirty-state flags; it does not preserve edited
  values. `keepDefaultValues` preserves the original comparison baseline.
- Do not carry dirty values from one record into another through a broad reset-preservation option.
  Define the unsaved-change and conflict policy for that transition.
- Use `resetField` for a registered field when only that field's value or baseline should reset.
  Supply a valid default value if changing its baseline.
- Keep the `errors` object passed into `useForm` reference-stable. Rebuilding it on every render can
  cause repeated updates.
- Depend on the specific method used by an Effect, such as `reset`, rather than the entire `useForm`
  ...
  runs on every form-state update.

See [reset options](https://react-hook-form.com/docs/useform/reset).

## Register inputs and controlled components

Use `register` for native inputs and components that forward the native input contract. Preserve its
...
- Give each logical field a stable, typed path. Checkbox and radio groups may share a logical name
  according to their registration contract; unrelated inputs must not overwrite the same field.
...
- Forward `field.value`, `field.onChange`, `field.onBlur`, `field.name`, `field.disabled`, and the
  ...
  library's `inputRef` or equivalent when necessary.
- Set controlled defaults at `useForm` where possible. Do not send `undefined` through
  `field.onChange`; choose an empty value supported by the control and schema.
- Send changes through `field.onChange`. Do not also call `setValue` for the same event. Use
  ...
  `shouldValidate` selected for that operation.
- When overriding an event or value conversion, spread `field` first and retain its remaining props.
  Losing `onBlur` breaks touched/blur validation; losing `ref` breaks error focus.
- Treat disabled values as potentially absent from submission. In particular, a disabled
  ...
  payload, and derive trusted identifiers again on the server.
- Manage file selection and cancellation deliberately. A `FileList` is not a normal text default;
  decide how empty selection, replacement, reset, and upload state behave.

See [Controller](https://react-hook-form.com/docs/usecontroller/controller).

## Define validation and parsed types

Choose one validation authority for the form. With `zodResolver`, express validation in the schema;
...
- Infer form input and successful submission output separately when coercion or transforms change
  ...
  the resolver's parsed output. Keep resolver `raw` mode explicit if intentionally returning input.
- Use async resolver behavior for async refinements or transforms. Do not force synchronous
  resolution for a schema that returns promises.
- Normalize empty numbers, checkboxes, dates, and repeated values once at the boundary that owns
  ...
  suffix. Do not silently turn a missing number into zero.
- For a numeric control, support its editing states before committing a domain value. An empty
  input, a minus sign, and a complete integer are different states.
...
- Use `criteriaMode: 'all'` only when the UI will display the extra issues. With built-in
  ...
  run alongside a configured resolver.
- Keep custom resolver errors hierarchical. A literal key such as `participants.1.name` is not a
  nested error object. Return both `values` and `errors`, with the shape the resolver API expects.
- Keep browser schemas free of server-only dependencies. Repeat validation and authorization at the
  server entry point; a valid client callback is not a trusted request.
...
Use `useWatch` near the component that displays a changing value and `useFormState` near the
...
- Use `getValues` for an imperative snapshot, not as a render subscription.
...
- Wait for `formState.isReady` before a child performs an initialization update that requires the
  parent's subscriptions. Distinguish readiness from async default loading.
- `dirtyFields` describes individual changes; use `isDirty` for the whole form. Choose whether a
  value restored to its default should still count as an unsaved change.
...
Use `useFieldArray` for editable rows with an object shape, such as
...
- Render each row with the generated `field.id` as its React key. Use the current index in the
  ...
  identifies its current location in the submitted array.
- Keep database IDs in a separate property such as `recordId`. Do not submit the generated render
  ...
  the generated key in `fields`, or use a supported custom `keyName` deliberately.
- Treat `fields` as the row structure and generated-key source, not a live value subscription. Read
  ...
  row metadata, preserve the generated key rather than overwriting it with a domain ID.
- Supply complete row values to `append`, `prepend`, `insert`, and `update`. Do not pass an empty
  object or omit required defaults. Use a row factory when several operations need the same shape.
- `update` unmounts and remounts the affected row. Use `setValue` on a specific leaf when preserving
  ...
  replacement and check the resulting row identity and focus behavior.
- Use `move` or `swap` for reordering and `remove(index)` for targeted deletion. Calling `remove()`
  without an index removes every row; make that intent explicit at the call site.
- Avoid stacking dependent array mutations in one event. Prefer a single operation that describes
  ...
  operation; do not add an unconditional mount Effect that deletes a row.
- Avoid `shouldUnregister: true` on forms or controllers that own field arrays. Reordering and
  remounting must not unregister and lose row values.
- Use built-in array `rules` only with the built-in validation path. Its array-level errors appear
  ...
  schema result instead of assuming that built-in path.
- On versions supporting field-array `disabled`, it disables the entire array and makes its mutation
  ...
  to the input. It is not a per-row flag supplied to `append`.
- Choose `shouldFocus`, `focusName`, or `focusIndex` for additions according to the interaction. A
  background update should not steal focus from the field being edited.

See [useFieldArray](https://react-hook-form.com/docs/usefieldarray).

## Preserve values across unmounts

Define whether hidden fields remain part of the draft and submission. The default
...
- For conditional fields, distinguish hidden-but-retained data from data that must be discarded.
  Keep hooks unconditional; extract a component for a branch that owns its own hooks.
...
- For virtualized forms, keep the form owner mounted while rows enter and leave the viewport. Retain
  values across row unmounts, supply complete defaults, and use stable generated row keys.
- Integrate with the existing Virtua setup instead of adding another virtualizer for forms.
- An offscreen invalid input cannot receive focus until its row is mounted. Resolve the error to the
  ...
  values while the row is absent.
- Do not copy `fields` or initial defaults back over current edits when a row remounts.
...
- Do not use `isSubmitSuccessful` alone as proof that a domain write succeeded. A callback that
  ...
  Reset or navigate only after an explicit successful server result.
...
- Keep pending, validation, and result state distinct. Avoid a second loading flag that can disagree
  with `isSubmitting`, a mutation's pending state, or an Action's pending state.

## Integrate Server Actions and useActionState

Choose either a direct awaited Server Action call from the valid submit handler or a
...
Do not mirror the Action's result or pending flag into local state through Effects. If it returns
...
- Use `fieldState.error` in controlled field components and the relevant nested error path in native
  field components. Keep parent/array errors visible as well as leaf errors.
- Use `setError` for returned field failures and a form-level key such as `root.server` for a safe
  operation error. Clear or replace those errors when their cause no longer applies.
- Do not treat `clearErrors` as revalidation. Use `trigger` when validation must run again, and
  await its result before an action that depends on validity.
- Treat `isValid` as validation state, not proof of authorization or a successful save. A manual
  error's effect on it can be replaced by the next validation run.
- Keep refs attached for `shouldFocusError` and `setFocus`. Do not focus immediately after a reset
  that has removed the input refs; wait for the relevant control to mount.
- Translate safe error codes and field messages through the existing internationalization layer. Do
  not display raw exceptions, SQL, submitted secrets, or resolver debug dumps.
- Use native validation only with compatible modes and actual input refs. Supply string messages and
  verify the browser's focus/reporting behavior with the custom controls in use.

## Compose form components

Use `FormProvider` and `useFormContext` when deeply nested descendants need the same form. Keep a
...
- With `createFormControl`, connect the returned `formControl` through `useForm({ formControl })`
  and pass its `control` to consuming hooks as needed. Use this explicit-control approach instead of
  adding a redundant `FormProvider` for the same control.
- Release external subscriptions on unmount or disposal. Reusing a control across editors must not
  carry one user's values, errors, or subscriptions into another editor.
...
Use the library's `FieldPath`, `FieldArrayPath`, `Control`, and related types for reusable form
...
- Preserve literal path types for indexed fields. Use an `as const` assertion when inference needs
  it; do not assert an unrelated valid path just to silence a type error.
- Model nested field arrays with their actual object shape. Keep runtime path construction aligned
  with the type and the current row index.
...
- On a release exposing `OpaqueTypes`, register an existing rich value type through module
  augmentation only when it should be a leaf in field-path inference. Put the declaration in an
  included module file and preserve the original `react-hook-form` module declarations.
- Opaque registration changes type traversal; it does not validate the value, make class instances
  ...
  avoids those runtime concerns.

## Test form behavior

Create, update, or run form tests only when explicitly requested. For requested tests, cover the
changed interaction using the existing runner and accessible queries. Await asynchronous validation
and submission results instead of adding sleeps. Keep coverage limited to the requested behavior.
