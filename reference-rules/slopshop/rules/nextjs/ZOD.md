...
Use these rules for runtime validation, schema composition, codecs, errors, and Zod upgrades.
...
- [Validate boundaries with Zod](#validate-boundaries-with-zod)
- [Choose supported Zod APIs](#choose-supported-zod-apis)
- [Parse external input](#parse-external-input)
- [Define objects and patch contracts](#define-objects-and-patch-contracts)
- [Normalize form and URL values](#normalize-form-and-url-values)
- [Compose schemas and refinements](#compose-schemas-and-refinements)
- [Encode and decode with codecs](#encode-and-decode-with-codecs)
- [Return useful validation errors](#return-useful-validation-errors)
- [Publish schemas and metadata](#publish-schemas-and-metadata)
- [Compile frequently used schemas](#compile-frequently-used-schemas)
- [Verify Zod upgrades](#verify-zod-upgrades)
...
- In Zod 4.5, use `.exactOptional()` or `.exactPartial()` when absence is allowed but an explicit
  `undefined` should fail. `.exactPartial()` preserves an inner schema's existing acceptance of
...
- Use `.partial()` for changes to the top-level fields. Use Zod 4.5 `z.deepPartial(schema)` only
  ...
  changes would weaken the contract.
...
- Select URL rules for the use case. `z.url()` accepts schemes beyond HTTP, while `z.httpUrl()`
  ...
  fetch. Check the destination against the operation’s permitted targets.
- Distinguish calendar dates, local wall-clock times, and instants. Choose datetime offset and
  ...
  not turn a timezone-less value into an instant without the application's timezone policy.
- Match length validation to the product's definition of a character. Zod 4.5 string length checks
  ...
  grapheme segmentation when the requirement concerns displayed characters.
...
- Treat recursive input support separately from transport support. Zod 4.5 can parse cyclic input;
  ...
  strict objects. Use an explicit transport shape for responses and persisted state.
...
- In Zod 4.5, runtime `z.input(schema)` and `z.output(schema)` select sides of nested pipes and
  ...
  schema. Check wrapper behavior when defaults or prefaults belong to only one side of a codec.
- Define the expected round trip. Normalizing a URL or datetime can preserve its meaning without
  ...
  the validation issue model when invalid input is expected.
...
- Use the Zod 4 `error` option for custom messages. Refinement/check messages can take precedence
  ...
  `undefined` from an error map when the next configured handler should decide.
- Translate issues at render time or pass a request-scoped error map when parsing. Error callbacks
  ...
  and other request-specific global configuration.
...
- Use `import "zod/compile"` only from an application-owned entry point, before modules construct
  ...
  a shared library should not enable global compilation for its consumers.
