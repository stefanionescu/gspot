...
The sequence in [02-cli.md](https://github.com/stefanionescu/gspot/blob/main/architecture/02-cli.md) is authoritative. `init` runs no check. Before changing or
deleting an existing file, it saves recoverable bytes and ownership metadata. Follow the contract
in [03-configuration.md](https://github.com/stefanionescu/gspot/blob/main/architecture/03-configuration.md).
...
[22-remaining.md](https://github.com/stefanionescu/gspot/blob/main/architecture/22-remaining.md). Rejected: a worktree-only deliverable that leaves the real
...
commands are authoritative in [22-remaining.md](https://github.com/stefanionescu/gspot/blob/main/architecture/22-remaining.md).
...
[16-file-tree.md](https://github.com/stefanionescu/gspot/blob/main/architecture/16-file-tree.md) replaces the previous mandatory filename inventory.
...
A fresh clone runs `gspot install`. A changed or conflicted manifest or lock needs `gspot apply`, review and commit, then `gspot install`. Full lifecycle and dry-run behavior are in [02-cli.md](https://github.com/stefanionescu/gspot/blob/main/architecture/02-cli.md) and [03-configuration.md](https://github.com/stefanionescu/gspot/blob/main/architecture/03-configuration.md). No lifecycle or setup script is injected (D-115).
...
A real upgrade preserves recovery first and updates the version pin only after the new config and generated files are valid. This is the explicit exception to refusing a mismatched pin. Unknown migrations fail without replacing originals. See [02-cli.md](https://github.com/stefanionescu/gspot/blob/main/architecture/02-cli.md) for failure and retry behavior.
...
No baseline or hidden-findings count changes the exit status. Existing defects can therefore block a push. Adoption fixes or explicitly ignores them; it does not silently discard them. [10-hooks-ci-runners.md](https://github.com/stefanionescu/gspot/blob/main/architecture/10-hooks-ci-runners.md) owns revision selection and exact snapshots.
...
Interactive and CI changed runs use this flag with the revision semantics of [10-hooks-ci-runners.md](https://github.com/stefanionescu/gspot/blob/main/architecture/10-hooks-ci-runners.md). The push hook instead uses the internal `check --push` protocol, with all local/remote ref pairs from stdin. `--since` goes, with no alias
...
[19-names.md](https://github.com/stefanionescu/gspot/blob/main/architecture/19-names.md) owns the vocabulary: preset name, check name, rule name, and setting name. Domain definitions use `name`, not interchangeable `id` or `key`; references keep the entity field such as `Finding.check`. K-308 specifies parameter roles, action names, `help` versus `fix_command`, and check coverage. Third-party wire formats keep their required fields.

[08-naming-policy.md](https://github.com/stefanionescu/gspot/blob/main/architecture/08-naming-policy.md) owns banned terms: only at `all`, with `generate` and `service` removed. Recommended findings require a demonstrated defect. A preferred abstraction or a house naming style does not qualify. Reference-project tests review the usefulness of findings, not only their count.

## D-175 Boundaries, ownership, carryover, and serialization

[03-configuration.md](https://github.com/stefanionescu/gspot/blob/main/architecture/03-configuration.md) owns these four contracts:
